---
title: "Monthly Satellite SO4 Analysis with Simmons Data - Revision #1 Final Analysis"
...
Violin plot wrapping boxplot to visualize low vs high SO45yrCensor_dich vs ADI
```{r}
...
Complete model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SO4_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=SO4, id=ID)
summary(coxPH_model2)
```

Complete model + dx_yr
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SO4_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=SO4, id=ID)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
SO4x <- SO4 %>% filter(!is.na(SO4_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat))

#Then survival function
surv1 <- Surv(SO4x$time_DeathTxCensor, SO4x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(SO4x$SO4_5yrPreCensor, df=3) + SO4x$age_dx + SO4x$sex + SO4x$smokeHx + SO4x$dich_Race + SO4x$ADI_nat)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
SO4x <- SO4 %>% filter(!is.na(SO4_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat) &!is.na(dx_yr))

#Then survival function
surv1 <- Surv(SO4x$time_DeathTxCensor, SO4x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(SO4x$SO4_5yrPreCensor, df=3) + SO4x$age_dx + SO4x$sex + SO4x$smokeHx + SO4x$dich_Race + SO4x$ADI_nat + SO4x$dx_yr)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Complete model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SO45yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=SO4, id=ID)
summary(coxPH_model8)
```

Complete model + dx_yr
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SO45yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=SO4, id=ID)
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ SO4_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=SO4)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ SO4_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=SO4)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ SO45yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=SO4)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model4 <- lm(fvc_pct ~ SO45yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=SO4)
summary(FVC_model4)
confint(FVC_model4)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ SO4_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=SO4)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ SO4_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=SO4)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ SO45yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=SO4)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ SO45yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=SO4)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SO4_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=SO4_IPF, id=ID)
summary(coxPH_model2)
```

Complete model + dx_yr
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SO4_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=SO4_IPF, id=ID)
summary(coxPH_model3)
```
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
SO4x <- SO4_IPF %>% filter(!is.na(SO4_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat))
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
SO4x <- SO4_IPF %>% filter(!is.na(SO4_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat) & !is.na(dx_yr))
...
Complete model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SO45yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=SO4_IPF, id=ID)
summary(coxPH_model8)
```

Complete model + dx_yr
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SO45yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=SO4_IPF, id=ID)
summary(coxPH_model9)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ SO4_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=SO4_IPF)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ SO4_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=SO4_IPF)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ SO45yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=SO4_IPF)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ SO45yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=SO4_IPF)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ SO4_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=SO4_IPF)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ SO4_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=SO4_IPF)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ SO45yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=SO4_IPF)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ SO45yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=SO4_IPF)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
coxPH_model2a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SO4_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015, id=ID)
summary(coxPH_model2a)
```

Complete model
```{r}
coxPH_model2b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SO4_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015, id=ID)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
pre2015x <- pre2015 %>% filter(!is.na(SO4_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat))

#Then survival function
surv1 <- Surv(pre2015x$time_DeathTxCensor, pre2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(pre2015x$SO4_5yrPreCensor, df=3) + pre2015x$age_dx + pre2015x$sex + pre2015x$smokeHx + pre2015x$dich_Race + pre2015x$ADI_nat)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
post2015x <- post2015 %>% filter(!is.na(SO4_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat))

#Then survival function
surv1 <- Surv(post2015x$time_DeathTxCensor, post2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(post2015x$SO4_5yrPreCensor, df=3) + post2015x$age_dx + post2015x$sex + post2015x$smokeHx + post2015x$dich_Race + post2015x$ADI_nat)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Complete model
```{r}
coxPH_model8a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SO45yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015, id=ID)
summary(coxPH_model8a)
```

Complete model
```{r}
coxPH_model8b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SO45yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015, id=ID)
...
Complete model
```{r}
FVC_model2a <- lm(fvc_pct ~ SO4_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015)
summary(FVC_model2a)
confint(FVC_model2a)
```
...
Complete model
```{r}
FVC_model2b <- lm(fvc_pct ~ SO4_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015)
summary(FVC_model2b)
confint(FVC_model2b)
```
...
Complete model
```{r}
FVC_model2a <- lm(fvc_pct ~ SO45yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015)
summary(FVC_model2a)
confint(FVC_model2a)
```
...
Complete model
```{r}
FVC_model2b <- lm(fvc_pct ~ SO45yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015)
summary(FVC_model2b)
confint(FVC_model2b)
```
...
Complete model
```{r}
DLCO_model2a <- lm(dlco_pct ~ SO4_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015)
...
Complete model
```{r}
DLCO_model2b <- lm(dlco_pct ~ SO4_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015)
...
Complete model
```{r}
DLCO_model2a <- lm(dlco_pct ~ SO45yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015)
summary(DLCO_model2a)
confint(DLCO_model2a)
```
...
Complete model
```{r}
DLCO_model2b <- lm(dlco_pct ~ SO45yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015)
summary(DLCO_model2b)
confint(DLCO_model2b)
```
