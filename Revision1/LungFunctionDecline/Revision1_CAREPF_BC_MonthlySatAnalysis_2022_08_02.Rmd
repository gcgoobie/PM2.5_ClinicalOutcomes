---
title: "Monthly Satellite BC Analysis with CARE-PF Data - Revision 1 Final Analysis"
...
```{r}
BC <- BC %>% 
  pivot_longer(cols=c(66:280), names_to="BC_date", names_prefix="BC_")
```
...
Violin plot wrapping boxplot to visualize low vs high BC5yrCensor_dich vs CIMD
```{r}
...
Complete model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=BC, id=ID)
...
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=BC, id=ID)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
BCx <- BC %>% filter(!is.na(BC_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(avg_s) & !is.na(site))

#Then survival function
surv1 <- Surv(BCx$time_DeathTxCensor, BCx$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(BCx$BC_5yrPreCensor, df=3) + BCx$age_dx + BCx$sex + BCx$smokeHx + BCx$dich_Race + BCx$avg_s + BCx$site)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
BCx <- BC %>% filter(!is.na(BC_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(avg_s) & !is.na(site) & !is.na(dx_yr))
...
Complete model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=BC, id=ID)
summary(coxPH_model8)
```

Complete model + dx_yr
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=BC, id=ID)
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=BC)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=BC)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=BC)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=BC)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=BC)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=BC)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=BC)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=BC)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=BC_IPF, id=ID)
summary(coxPH_model2)
```
...
Complete model + dx_yr
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=BC_IPF, id=ID)
summary(coxPH_model3)
```
...
#plot(coxPH_model3_tvc)
```
Error with the bove code "Error in optim(par = c(BC_5yrPreCensor = 4.98043221986785, sexF = -0.272936265634474, : 
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
BCx <- BC_IPF %>% filter(!is.na(BC_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(avg_s) & !is.na(site))
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
BCx <- BC_IPF %>% filter(!is.na(BC_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(avg_s) & !is.na(site) & !is.na(dx_yr))

#Then survival function
surv1 <- Surv(BCx$time_DeathTxCensor, BCx$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(BCx$BC_5yrPreCensor, df=4) + BCx$age_dx + BCx$sex + BCx$smokeHx + BCx$dich_Race + BCx$avg_s + BCx$site + BCx$dx_yr)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Complete model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=BC_IPF, id=ID)
summary(coxPH_model8)
```
...
Complete model + dx_yr
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=BC_IPF, id=ID)
summary(coxPH_model9)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=BC_IPF)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=BC_IPF)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=BC_IPF)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=BC_IPF)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=BC_IPF)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=BC_IPF)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=BC_IPF)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=BC_IPF)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
coxPH_model2a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=pre2015, id=ID)
summary(coxPH_model2a)
```

Complete model
```{r}
coxPH_model2b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=post2015, id=ID)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
pre2015x <- pre2015 %>% filter(!is.na(BC_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(avg_s) & !is.na(site))

#Then survival function
surv1 <- Surv(pre2015x$time_DeathTxCensor, pre2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(pre2015x$BC_5yrPreCensor, df=3) + pre2015x$age_dx + pre2015x$sex + pre2015x$smokeHx + pre2015x$dich_Race + pre2015x$avg_s + pre2015x$site)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
post2015x <- post2015 %>% filter(!is.na(BC_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(avg_s) & !is.na(site))

#Then survival function
surv1 <- Surv(post2015x$time_DeathTxCensor, post2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(post2015x$BC_5yrPreCensor, df=3) + post2015x$age_dx + post2015x$sex + post2015x$smokeHx + post2015x$dich_Race + post2015x$avg_s + post2015x$site)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Complete model
```{r}
coxPH_model8a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=pre2015, id=ID)
summary(coxPH_model8a)
```

Complete model
```{r}
coxPH_model8b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=post2015, id=ID)
...
Complete model
```{r}
FVC_model2a <- lm(fvc_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=pre2015)
summary(FVC_model2a)
confint(FVC_model2a)
```
...
Complete model
```{r}
FVC_model2b <- lm(fvc_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=post2015)
summary(FVC_model2b)
confint(FVC_model2b)
```
...
Complete model
```{r}
FVC_model2a <- lm(fvc_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=pre2015)
summary(FVC_model2a)
confint(FVC_model2a)
```
...
Complete model
```{r}
FVC_model2b <- lm(fvc_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=post2015)
summary(FVC_model2b)
confint(FVC_model2b)
```
...
Complete model
```{r}
DLCO_model2a <- lm(dlco_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=pre2015)
...
Complete model
```{r}
DLCO_model2b <- lm(dlco_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=post2015)
...
Complete model
```{r}
DLCO_model2a <- lm(dlco_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=pre2015)
summary(DLCO_model2a)
confint(DLCO_model2a)
```
...
Complete model
```{r}
DLCO_model2b <- lm(dlco_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=post2015)
summary(DLCO_model2b)
confint(DLCO_model2b)
```
