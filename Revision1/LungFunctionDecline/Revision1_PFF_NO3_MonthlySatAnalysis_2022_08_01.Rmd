---
title: "Monthly Satellite NO3 Analysis with Complete PFF Data - Revision 1 Final Analysis"
...
Then joing PFF to NO3
```{r}
NO3 <- left_join(NO3, PFF, by=c("ID"))
```
This results in 1905 complete records

Reorder so "ID" is the first column
```{r}
NO3 <- NO3 %>% dplyr::select(ID, SSID, everything(.))
```
...
```{r}
NO3 <- NO3 %>% 
  pivot_longer(cols=c(7:222), names_to="NO3_date", names_prefix="NIT_")
```
...
Cutoffs are based on calculations performed where the median total NO3 mass across all three cohorts was averaged, then the median constituent component proportion of total NO3 across all three cohorts was averaged. Subsequently, the average proportion of each constituent across the three cohorts was multiplied by the average of the total NO3 mass medians across the three cohorts to get the approximate median total mass of each constituent across all three cohorts, which was used to define the low vs high exposure cut points.
...
```{r}
summary(NO3$med_income_dollars)
sd(NO3$med_income_dollars, na.rm=T)
```

## Percent Below Poverty Level Breakdown
```{r}
summary(NO3$pct_belowpoverty)
sd(NO3$pct_belowpoverty, na.rm=T)
```
...
Complete model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=NO3, id=ID)
...
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + dx_yr + pct_belowpoverty + site, data=NO3, id=ID)
...
fit1 <- coxph(surv1 ~ pspline(NO3x$NO3_5yrPreCensor, df=3))
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)

#Then plot
plot(NO3x$NO3_5yrPreCensor, exp(predicted$fit), type="n", xlim=c(0,4), ylim=c(0,5))
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
NO3x <- NO3 %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race)& !is.na(pct_belowpoverty) & !is.na(site))

#Then survival function
surv1 <- Surv(NO3x$time_DeathTxCensor, NO3x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(NO3x$NO3_5yrPreCensor, df=3) + NO3x$age_dx + NO3x$sex + NO3x$smokeHx + NO3x$dich_Race+ NO3x$pct_belowpoverty + NO3x$site)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
NO3x <- NO3 %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race)& !is.na(pct_belowpoverty) & !is.na(site) & !is.na(dx_yr))

#Then survival function
surv1 <- Surv(NO3x$time_DeathTxCensor, NO3x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(NO3x$NO3_5yrPreCensor, df=3) + NO3x$age_dx + NO3x$sex + NO3x$smokeHx + NO3x$dich_Race+ NO3x$pct_belowpoverty + NO3x$site + NO3x$dx_yr)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Complete model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=NO3, id=ID)
...
Complete model + dx_yr
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site + dx_yr, data=NO3, id=ID)
summary(coxPH_model9)
```


Now I will make a Kaplan-Meier survival curve, discretized by NO3 during disease low vs high:
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=NO3)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site + dx_yr, data=NO3)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=NO3)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site + dx_yr, data=NO3)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=NO3)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site + dx_yr, data=NO3)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=NO3)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site + dx_yr, data=NO3)
summary(DLCO_model3)
confint(DLCO_model3)
```


# IPF-Only - Cox PH Models for Association between NO3 and Death or Lung Transplant
## IPF-Only - NO3 5yrs Pre-Censoring Models
### IPF-Only - Continuous NO3 5yrs pre-censoring until Death/Transplant/Censoring
Start with the simplest model just looking at NO3 in the year of and 5yrs prior to death/transplant/censoring
```{r}
coxPH_model1 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor, data=NO3_IPF, id=ID)
summary(coxPH_model1)
```
...
Complete model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=NO3_IPF, id=ID)
summary(coxPH_model2)
```
...
Complete model + dx_yr
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site + dx_yr, data=NO3_IPF, id=ID)
summary(coxPH_model3)
```
...
### IPF-Only - Variable HR for NO3 risk in 5yr Pre-Cenosr
Want a plot with x-axis of NO3 level and y-axis of HR

Unadjusted model
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
NO3x <- NO3_IPF %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor))
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
NO3x <- NO3_IPF %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race)& !is.na(pct_belowpoverty) & !is.na(site))
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
NO3x <- NO3_IPF %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(dx_yr)& !is.na(pct_belowpoverty) & !is.na(site))

#Then survival function
surv1 <- Surv(NO3x$time_DeathTxCensor, NO3x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(NO3x$NO3_5yrPreCensor, df=3) + NO3x$age_dx + NO3x$sex + NO3x$smokeHx + NO3x$dich_Race + NO3x$dx_yr+ NO3x$pct_belowpoverty + NO3x$site)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
### IPF-Only - NO3 Low vs High 5yrs Pre-Censoring
Base Model:
```{r}
coxPH_model7 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich, data=NO3_IPF, id=ID)
summary(coxPH_model7)
```
...
Complete model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=NO3_IPF, id=ID)
summary(coxPH_model8)
```
...
Complete model + dx_yr
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site + dx_yr, data=NO3_IPF, id=ID)
summary(coxPH_model9)
```
...
Now I will make a Kaplan-Meier survival curve, discretized by NO3 during disease low vs high:
```{r, fig.height=5, fig.width=7}
surv1 <- survfit(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich, data=NO3_IPF)
(ggsurvplot(surv1, 
            palette=c("dodgerblue4","firebrick4"),
            censor=TRUE,
            pval=TRUE,
...
# IPF-Only - Association between NO3 and Baseline FVC
...
## IPF-Only - NO3 5yrs Pre-Diagnosis and FVC Models
Looking at continuous NO3_5yrPreDx vs baseline FVC

Simplest model just looking at NO3 in 5yrs pre-diagnosis
```{r}
FVC_model1 <- lm(fvc_pct ~ NO3_5yrPreDx, data=NO3_IPF)
summary(FVC_model1)
confint(FVC_model1)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=NO3_IPF)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site + dx_yr, data=NO3_IPF)
summary(FVC_model3)
confint(FVC_model3)
```


### IPF-Only - NO3 5yrs Pre-Diagnosis Low vs High and Baseline FVC
Looking at NO35yr_dich vs baseline FVC

Simplest model just looking at NO3 in 5yrs pre-diagnosis
```{r}
FVC_model1 <- lm(fvc_pct ~ NO35yr_dich, data=NO3_IPF)
summary(FVC_model1)
confint(FVC_model1)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=NO3_IPF)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site + dx_yr, data=NO3_IPF)
summary(FVC_model3)
confint(FVC_model3)
```


# IPF-Only - Association between NO3 and Baseline DLCO
## IPF-Only - Visual EDA 5yrs pre-Dx
Scatterplot of continuous NO3 5yrs pre-diagnosis vs DLCO, with sex represented by the color of points
```{r}
(NO3 %>% ggplot(aes(x=NO3_5yrPreDx, y=dlco_pct, color=sex))+
...
## IPF-Only - NO3 5yrs Pre-Diagnosis and DLCO Models
### IPF-Only - NO3 5yrs Pre-Diagnosis and Baseline DLCO
Looking at continuous NO3_5yrPreDx vs baseline DLCO

Simplest model just looking at NO3 in 5yrs pre-diagnosis
```{r}
DLCO_model1 <- lm(dlco_pct ~ NO3_5yrPreDx, data=NO3_IPF)
summary(DLCO_model1)
confint(DLCO_model1)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=NO3_IPF)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site + dx_yr, data=NO3_IPF)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Simplest model just looking at NO3 in 5yrs pre-diagnosis
```{r}
DLCO_model1 <- lm(dlco_pct ~ NO35yr_dich, data=NO3_IPF)
summary(DLCO_model1)
confint(DLCO_model1)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=NO3_IPF)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site + dx_yr, data=NO3_IPF)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
```{r}
pre2015 <- NO3 %>% filter(dx_yr<=2015)

post2015 <- NO3 %>% filter(dx_yr>2015)
```

# Pre/Post-2015 Only - Cox PH Models for Association between NO3 and Death or Lung Transplant
## Pre/Post-2015 Only - NO3 5yrs Pre-Censoring Models
### Pre/Post-2015 Only - Continuous NO3 5yrs pre-censoring until Death/Transplant/Censoring
Start with the simplest model just looking at NO3 in the year of and 5yrs prior to death/transplant/censoring
```{r}
coxPH_model1a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor, data=pre2015, id=ID)
summary(coxPH_model1a)
```

```{r}
coxPH_model1b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor, data=post2015, id=ID)
summary(coxPH_model1b)
```

Complete model
```{r}
coxPH_model2a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=pre2015, id=ID)
summary(coxPH_model2a)
```

Complete model
```{r}
coxPH_model2b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=post2015, id=ID)
summary(coxPH_model2b)
```

### Pre/Post-2015 Only - Variable HR for NO3 risk in 5yr Pre-Cenosr
Want a plot with x-axis of NO3 level and y-axis of HR

Unadjusted model - Pre-2015
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
pre2015x <- pre2015 %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor))

#Then make survival function
surv1 <- Surv(pre2015x$time_DeathTxCensor, pre2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(pre2015x$NO3_5yrPreCensor, df=3))
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)

#Then plot
plot(pre2015x$NO3_5yrPreCensor, exp(predicted$fit), type="n", xlim=c(0,4), ylim=c(0,5))
lines(sm.spline(pre2015x$NO3_5yrPreCensor, exp(predicted$fit)), col = "red" , lty = 1 )
lines(sm.spline(pre2015x$NO3_5yrPreCensor, exp(predicted$fit + 1.96 * predicted$se)), col = "orange" , lty = 2 )
lines(sm.spline(pre2015x$NO3_5yrPreCensor, exp(predicted$fit - 1.96 * predicted$se)), col = "orange" , lty = 2 )
```
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
post2015x <- post2015 %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor))

#Then make survival function
surv1 <- Surv(post2015x$time_DeathTxCensor, post2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(post2015x$NO3_5yrPreCensor, df=3))
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)

#Then plot
plot(post2015x$NO3_5yrPreCensor, exp(predicted$fit), type="n", xlim=c(0,4), ylim=c(0,5))
lines(sm.spline(post2015x$NO3_5yrPreCensor, exp(predicted$fit)), col = "red" , lty = 1 )
lines(sm.spline(post2015x$NO3_5yrPreCensor, exp(predicted$fit + 1.96 * predicted$se)), col = "orange" , lty = 2 )
lines(sm.spline(post2015x$NO3_5yrPreCensor, exp(predicted$fit - 1.96 * predicted$se)), col = "orange" , lty = 2 )
```
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
pre2015x <- pre2015 %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race)& !is.na(pct_belowpoverty) & !is.na(site))

#Then survival function
surv1 <- Surv(pre2015x$time_DeathTxCensor, pre2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(pre2015x$NO3_5yrPreCensor, df=3) + pre2015x$age_dx + pre2015x$sex + pre2015x$smokeHx + pre2015x$dich_Race+ pre2015x$pct_belowpoverty + pre2015x$site)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
post2015x <- post2015 %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race)& !is.na(pct_belowpoverty) & !is.na(site))

#Then survival function
surv1 <- Surv(post2015x$time_DeathTxCensor, post2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(post2015x$NO3_5yrPreCensor, df=3) + post2015x$age_dx + post2015x$sex + post2015x$smokeHx + post2015x$dich_Race+ post2015x$pct_belowpoverty + post2015x$site)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
### Pre/Post-2015 Only - NO3 Low vs High 5yrs Pre-Censoring
Base Model:
```{r}
coxPH_model7a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich, data=pre2015, id=ID)
summary(coxPH_model7a)
```

Base Model:
```{r}
coxPH_model7b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich, data=post2015, id=ID)
summary(coxPH_model7b)
```

Complete model
```{r}
coxPH_model8a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=pre2015, id=ID)
summary(coxPH_model8a)
```

Complete model
```{r}
coxPH_model8b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=post2015, id=ID)
...
```{r, fig.height=5, fig.width=7}
surv1 <- survfit(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich, data=pre2015)
(ggsurvplot(surv1, 
            palette=c("dodgerblue4","firebrick4"),
            censor=TRUE,
            pval=TRUE,
...
Now I will make a Kaplan-Meier survival curve, discretized by NO3 during disease deciles:
```{r, fig.height=5, fig.width=7}
surv1 <- survfit(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich, data=post2015)
(ggsurvplot(surv1, 
            palette=c("dodgerblue4","firebrick4"),
            censor=TRUE,
            pval=TRUE,
...
# Pre/Post-2015 Only - Association between NO3 and Baseline FVC
## Pre/Post-2015 Only - Visual EDA 5yrs pre-Dx
Scatterplot of continuous NO3 5yrs pre-diagnosis vs FVC, with sex represented by the color of points - Pre-2015
```{r}
(pre2015 %>% ggplot(aes(x=NO3_5yrPreDx, y=fvc_pct, color=sex))+
   geom_point()+
   labs(x="NO3 5yrs Pre-Diagnosis", y="Baseline FVC % Predicted", title="Baseline FVC % Predicted by NO3 5yrs Pre-Diagnosis")+
   geom_smooth(method="lm", se = FALSE) +
   theme(plot.title = element_text(hjust = 0.5))+
...
Scatterplot of continuous NO3 5yrs pre-diagnosis vs FVC, with sex represented by the color of points - Post-2015
```{r}
(post2015 %>% ggplot(aes(x=NO3_5yrPreDx, y=fvc_pct, color=sex))+
   geom_point()+
   labs(x="NO3 5yrs Pre-Diagnosis", y="Baseline FVC % Predicted", title="Baseline FVC % Predicted by NO3 5yrs Pre-Diagnosis")+
   geom_smooth(method="lm", se = FALSE) +
   theme(plot.title = element_text(hjust = 0.5))+
...
Violin plot wrapping boxplot to visualize NO3 5yrs pre-dx low vs high vs FVC - Pre-2015
```{r}
pre2015b <- pre2015 %>% filter(!is.na(NO35yr_dich))
(pre2015b %>% ggplot(aes(x=NO35yr_dich, y=fvc_pct, fill=NO35yr_dich))+
   geom_boxplot(width=0.2, color="black", alpha=1.0)+
   geom_violin(width=1.0, alpha=0.5)+
   labs(x="NO3 5yrs Pre-Diagnosis Low vs High", y="Baseline FVC % Predicted", title="Baseline FVC % Predicted by NO3 5yrs Pre-Diagnosis Low vs High")+
   theme_light()+
...
Violin plot wrapping boxplot to visualize NO3 5yrs pre-dx low vs high vs FVC - Post-2015
```{r}
post2015b <- post2015 %>% filter(!is.na(NO35yr_dich))
(post2015b %>% ggplot(aes(x=NO35yr_dich, y=fvc_pct, fill=NO35yr_dich))+
   geom_boxplot(width=0.2, color="black", alpha=1.0)+
   geom_violin(width=1.0, alpha=0.5)+
   labs(x="NO3 5yrs Pre-Diagnosis Low vs High", y="Baseline FVC % Predicted", title="Baseline FVC % Predicted by NO3 5yrs Pre-Diagnosis Low vs High")+
   theme_light()+
...
## Pre/Post-2015 Only - NO3 5yrs Pre-Diagnosis and FVC Models
Looking at continuous NO3_5yrPreDx vs baseline FVC

Simplest model just looking at NO3 in 5yrs pre-diagnosis
```{r}
FVC_model1a <- lm(fvc_pct ~ NO3_5yrPreDx, data=pre2015)
summary(FVC_model1a)
confint(FVC_model1a)
```
...
Simplest model just looking at NO3 in 5yrs pre-diagnosis
```{r}
FVC_model1b <- lm(fvc_pct ~ NO3_5yrPreDx, data=post2015)
summary(FVC_model1b)
confint(FVC_model1b)
```
...
Complete model
```{r}
FVC_model2a <- lm(fvc_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=pre2015)
summary(FVC_model2a)
confint(FVC_model2a)
```
...
Complete model
```{r}
FVC_model2b <- lm(fvc_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=post2015)
summary(FVC_model2b)
confint(FVC_model2b)
```


### Pre/Post-2015 Only - NO3 5yrs Pre-Diagnosis Low vs High and Baseline FVC
Looking at NO35yr_dich vs baseline FVC

Simplest model just looking at NO3 in 5yrs pre-diagnosis
```{r}
FVC_model1a <- lm(fvc_pct ~ NO35yr_dich, data=pre2015)
summary(FVC_model1a)
confint(FVC_model1a)
```
...
Simplest model just looking at NO3 in 5yrs pre-diagnosis
```{r}
FVC_model1b <- lm(fvc_pct ~ NO35yr_dich, data=post2015)
summary(FVC_model1b)
confint(FVC_model1b)
```
...
Complete model
```{r}
FVC_model2a <- lm(fvc_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=pre2015)
summary(FVC_model2a)
confint(FVC_model2a)
```
...
Complete model
```{r}
FVC_model2b <- lm(fvc_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=post2015)
summary(FVC_model2b)
confint(FVC_model2b)
```

# Pre/Post-2015 Only - Association between NO3 and Baseline DLCO
## Pre/Post-2015 Only - Visual EDA 5yrs pre-Dx
Scatterplot of continuous NO3 5yrs pre-diagnosis vs DLCO, with sex represented by the color of points - Pre-2015
```{r}
(pre2015 %>% ggplot(aes(x=NO3_5yrPreDx, y=dlco_pct, color=sex))+
   geom_point()+
   labs(x="NO3 5yrs Pre-Diagnosis", y="Baseline DLCO % Predicted", title="Baseline DLCO % Predicted by NO3 5yrs Pre-Diagnosis")+
   geom_smooth(method="lm", se = FALSE) +
   theme(plot.title = element_text(hjust = 0.5))+
...
Scatterplot of continuous NO3 5yrs pre-diagnosis vs DLCO, with sex represented by the color of points - Post-2015
```{r}
(post2015 %>% ggplot(aes(x=NO3_5yrPreDx, y=dlco_pct, color=sex))+
   geom_point()+
   labs(x="NO3 5yrs Pre-Diagnosis", y="Baseline DLCO % Predicted", title="Baseline DLCO % Predicted by NO3 5yrs Pre-Diagnosis")+
   geom_smooth(method="lm", se = FALSE) +
   theme(plot.title = element_text(hjust = 0.5))+
...
Violin plot wrapping boxplot to visualize NO3 5yrs pre-dx low vs high vs DLCO - Pre-2015
```{r}
(pre2015b %>% ggplot(aes(x=NO35yr_dich, y=dlco_pct, fill=NO35yr_dich))+
   geom_boxplot(width=0.2, color="black", alpha=1.0)+
   geom_violin(width=1.0, alpha=0.5)+
   labs(x="NO3 5yrs Pre-Diagnosis Low vs High", y="Baseline DLCO % Predicted", title="Baseline DLCO % Predicted by NO3 5yrs Pre-Diagnosis Low vs High")+
   theme_light()+
...
Violin plot wrapping boxplot to visualize NO3 5yrs pre-dx low vs high vs DLCO - Post-2015
```{r}
(post2015b %>% ggplot(aes(x=NO35yr_dich, y=dlco_pct, fill=NO35yr_dich))+
   geom_boxplot(width=0.2, color="black", alpha=1.0)+
   geom_violin(width=1.0, alpha=0.5)+
   labs(x="NO3 5yrs Pre-Diagnosis Low vs High", y="Baseline DLCO % Predicted", title="Baseline DLCO % Predicted by NO3 5yrs Pre-Diagnosis Low vs High")+
   theme_light()+
...
## Pre/Post-2015 Only - NO3 5yrs Pre-Diagnosis and DLCO Models
### Pre/Post-2015 Only - NO3 5yrs Pre-Diagnosis and Baseline DLCO
Looking at continuous NO3_5yrPreDx vs baseline DLCO

Simplest model just looking at NO3 in 5yrs pre-diagnosis
```{r}
DLCO_model1a <- lm(dlco_pct ~ NO3_5yrPreDx, data=pre2015)
...
Simplest model just looking at NO3 in 5yrs pre-diagnosis
```{r}
DLCO_model1b <- lm(dlco_pct ~ NO3_5yrPreDx, data=post2015)
...
Complete model
```{r}
DLCO_model2a <- lm(dlco_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=pre2015)
...
Complete model
```{r}
DLCO_model2b <- lm(dlco_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=post2015)
summary(DLCO_model2b)
confint(DLCO_model2b)
```

### Pre/Post-2015 Only - NO3 5yrs Pre-Diagnosis Low vs High and Baseline DLCO
Looking at NO35yr_dich vs baseline DLCO

Simplest model just looking at NO3 in 5yrs pre-diagnosis
```{r}
DLCO_model1a <- lm(dlco_pct ~ NO35yr_dich, data=pre2015)
summary(DLCO_model1a)
confint(DLCO_model1a)
```
...
Simplest model just looking at NO3 in 5yrs pre-diagnosis
```{r}
DLCO_model1b <- lm(dlco_pct ~ NO35yr_dich, data=post2015)
summary(DLCO_model1b)
confint(DLCO_model1b)
```
...
Complete model
```{r}
DLCO_model2a <- lm(dlco_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=pre2015)
summary(DLCO_model2a)
confint(DLCO_model2a)
```
...
Complete model
```{r}
DLCO_model2b <- lm(dlco_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + pct_belowpoverty + site, data=post2015)
summary(DLCO_model2b)
confint(DLCO_model2b)
```
