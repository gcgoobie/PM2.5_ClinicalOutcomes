---
title: "Monthly Satellite SS Analysis with CARE-PF Data - Revision 1 Final Analysis"
...
# Merging Datasets
I used a inner_join here so that the complete "SS" dataframe only includes patients with fILD that have all baseline demographics and CIMD available as well as SS data.
```{r}
SS <- inner_join(CARE, SS, by="ID")
```
...
```{r}
SS <- SS %>% 
  pivot_longer(cols=c(66:280), names_to="SS_date", names_prefix="SS_")
```
...
```{r}
SS_toronto <- SS %>% filter(site==101)
SS_ab <- SS %>% filter(site==102)
SS_sph <- SS %>% filter(site==103)
SS_vgh <- SS %>%  filter(site==104)
SS_bc <- SS %>% filter(site==103 | site==104)
SS_laval <- SS %>% filter(site==105)
SS_hamilton <- SS %>% filter(site==106)
SS_on <- SS %>% filter(site==101 | site==106)
SS_mcgill <- SS %>% filter(site==107)
SS_qc <- SS %>% filter(site==105 | site==107)
SS_sk <- SS %>% filter(site==108)
```


# Export Excel File with SS Data Matched to Patients and Full Cohort Data
```{r}
write_xlsx(SS, path="Final_CAREPF_fILD_SS_BaselineData_2022_02_12.xlsx")
```
...
table(SS$province, SS$SS5yrCensor_dich)
...
table(SS$province, SS$SS5yr_dich)
...
Violin plot wrapping boxplot to visualize low vs high SS5yrCensor_dich vs CIMD
```{r}
(SS %>% ggplot(aes(x=SS5yrCensor_dich, y=avg_s, fill=SS5yrCensor_dich))+
   geom_boxplot(width=0.2, color="black", alpha=1.0)+
   geom_violin(width=1.0, alpha=0.5)+
   labs(x="Low vs High SS in 5yrs Pre-Censoring", y="National CIMD", title="National CIMD by SS Exposure")+
   theme(plot.title = element_text(hjust = 0.5))+
   ylim(-2.5,2.5))
```
...
```{r}
(SS %>% ggplot(aes(x=dx_date, y=SS_5yrPreCensor, color=site))+
...
Scatterplot showing average SS during 5yrs pre-diagnosis level by year of diagnosis
```{r}
(SS %>% ggplot(aes(x=dx_date, y=SS_5yrPreDx))+
   geom_point()+
   labs(x="Date of Diagnosis", y="SS Exposure During 5yrs Pre-Diagnosis", title="Pre-Diagnosis SS vs Date of Diagnosis")+
   geom_smooth(method="lm", se = FALSE) +
   theme(plot.title = element_text(hjust = 0.5))+
   ylim(0,20)+
...
Scatterplot showing average SS during 5yrs pre-diagnosis level by year of diagnosis
```{r}
(SS %>% ggplot(aes(x=dx_date, y=SS_5yrPreDx, color=site))+
   geom_point(alpha=0.5)+
   labs(x="Date of Diagnosis", y="SS Exposure During 5yrs Pre-Diagnosis", title="Pre-Diagnosis SS vs Date of Diagnosis")+
   geom_smooth(method="lm", se = FALSE) +
   theme(plot.title = element_text(hjust = 0.5))+
...
Complete model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SS_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=SS, id=ID)
...
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SS_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=SS, id=ID)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
SSx <- SS %>% filter(!is.na(SS_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(avg_s) & !is.na(site))

#Then survival function
surv1 <- Surv(SSx$time_DeathTxCensor, SSx$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(SSx$SS_5yrPreCensor, df=3) + SSx$age_dx + SSx$sex + SSx$smokeHx + SSx$dich_Race + SSx$avg_s + SSx$site)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
SSx <- SS %>% filter(!is.na(SS_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(avg_s) & !is.na(site) & !is.na(dx_yr))
...
Complete model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SS5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=SS, id=ID)
summary(coxPH_model8)
```

Complete model + dx_yr
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SS5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=SS, id=ID)
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ SS_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=SS)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ SS_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=SS)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ SS5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=SS)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ SS5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=SS)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ SS_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=SS)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ SS_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=SS)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ SS5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=SS)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ SS5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=SS)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SS_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=SS_IPF, id=ID)
summary(coxPH_model2)
```
...
Complete model + dx_yr
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SS_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=SS_IPF, id=ID)
summary(coxPH_model3)
```
...
#plot(coxPH_model3_tvc)
```
Error with the bove code "Error in optim(par = c(SS_5yrPreCensor = 4.98043221986785, sexF = -0.272936265634474, : 
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
SSx <- SS_IPF %>% filter(!is.na(SS_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(avg_s) & !is.na(site))
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
SSx <- SS_IPF %>% filter(!is.na(SS_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(avg_s) & !is.na(site) & !is.na(dx_yr))

#Then survival function
surv1 <- Surv(SSx$time_DeathTxCensor, SSx$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(SSx$SS_5yrPreCensor, df=4) + SSx$age_dx + SSx$sex + SSx$smokeHx + SSx$dich_Race + SSx$avg_s + SSx$site + SSx$dx_yr)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Complete model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SS5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=SS_IPF, id=ID)
summary(coxPH_model8)
```
...
Complete model + dx_yr
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SS5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=SS_IPF, id=ID)
summary(coxPH_model9)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ SS_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=SS_IPF)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ SS_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=SS_IPF)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ SS5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=SS_IPF)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ SS5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=SS_IPF)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ SS_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=SS_IPF)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ SS_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=SS_IPF)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ SS5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=SS_IPF)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ SS5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site + dx_yr, data=SS_IPF)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
coxPH_model2a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SS_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=pre2015, id=ID)
summary(coxPH_model2a)
```

Complete model
```{r}
coxPH_model2b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SS_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=post2015, id=ID)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
pre2015x <- pre2015 %>% filter(!is.na(SS_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(avg_s) & !is.na(site))

#Then survival function
surv1 <- Surv(pre2015x$time_DeathTxCensor, pre2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(pre2015x$SS_5yrPreCensor, df=3) + pre2015x$age_dx + pre2015x$sex + pre2015x$smokeHx + pre2015x$dich_Race + pre2015x$avg_s + pre2015x$site)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
post2015x <- post2015 %>% filter(!is.na(SS_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(avg_s) & !is.na(site))

#Then survival function
surv1 <- Surv(post2015x$time_DeathTxCensor, post2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(post2015x$SS_5yrPreCensor, df=3) + post2015x$age_dx + post2015x$sex + post2015x$smokeHx + post2015x$dich_Race + post2015x$avg_s + post2015x$site)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Complete model
```{r}
coxPH_model8a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SS5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=pre2015, id=ID)
summary(coxPH_model8a)
```

Complete model
```{r}
coxPH_model8b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SS5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=post2015, id=ID)
...
Complete model
```{r}
FVC_model2a <- lm(fvc_pct ~ SS_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=pre2015)
summary(FVC_model2a)
confint(FVC_model2a)
```
...
Complete model
```{r}
FVC_model2b <- lm(fvc_pct ~ SS_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=post2015)
summary(FVC_model2b)
confint(FVC_model2b)
```
...
Complete model
```{r}
FVC_model2a <- lm(fvc_pct ~ SS5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=pre2015)
summary(FVC_model2a)
confint(FVC_model2a)
```
...
Complete model
```{r}
FVC_model2b <- lm(fvc_pct ~ SS5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=post2015)
summary(FVC_model2b)
confint(FVC_model2b)
```
...
Complete model
```{r}
DLCO_model2a <- lm(dlco_pct ~ SS_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=pre2015)
...
Complete model
```{r}
DLCO_model2b <- lm(dlco_pct ~ SS_5yrPreDx + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=post2015)
...
Complete model
```{r}
DLCO_model2a <- lm(dlco_pct ~ SS5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=pre2015)
summary(DLCO_model2a)
confint(DLCO_model2a)
```
...
Complete model
```{r}
DLCO_model2b <- lm(dlco_pct ~ SS5yr_dich + sex + age_dx + smokeHx + dich_Race + avg_s + site, data=post2015)
summary(DLCO_model2b)
confint(DLCO_model2b)
```
