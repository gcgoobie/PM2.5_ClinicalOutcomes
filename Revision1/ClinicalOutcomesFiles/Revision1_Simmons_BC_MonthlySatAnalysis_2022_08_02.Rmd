---
title: "Monthly Satellite BC Analysis with Simmons Data - Revision #1 Final Analysis"
...
Cutoffs are based on calculations performed where the median total BC mass across all three cohorts was averaged, then the median constituent component proportion of total BC across all three cohorts was averaged. Subsequently, the average proportion of each constituent across the three cohorts was multiplied by the average of the total BC mass medians across the three cohorts to get the approximate median total mass of each constituent across all three cohorts, which was used to define the low vs high exposure cut points.
...
```{r}
write_xlsx(BC, path="Final_Simmons_fILD_BC_BaselineData_2022_03_28.xlsx")
```
...
Violin plot wrapping boxplot to visualize low vs high BC5yrCensor_dich vs ADI
```{r}
...
Complete model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=BC, id=ID)
summary(coxPH_model2)
```

Complete model + dx_yr
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=BC, id=ID)
...
fit1 <- coxph(surv1 ~ pspline(BCx$BC_5yrPreCensor, df=3))
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)

#Then plot
plot(BCx$BC_5yrPreCensor, exp(predicted$fit), type="n", xlim=c(0,1.75), ylim=c(0,5))
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
BCx <- BC %>% filter(!is.na(BC_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat))

#Then survival function
surv1 <- Surv(BCx$time_DeathTxCensor, BCx$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(BCx$BC_5yrPreCensor, df=3) + BCx$age_dx + BCx$sex + BCx$smokeHx + BCx$dich_Race + BCx$ADI_nat)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
BCx <- BC %>% filter(!is.na(BC_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat) &!is.na(dx_yr))

#Then survival function
surv1 <- Surv(BCx$time_DeathTxCensor, BCx$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(BCx$BC_5yrPreCensor, df=3) + BCx$age_dx + BCx$sex + BCx$smokeHx + BCx$dich_Race + BCx$ADI_nat + BCx$dx_yr)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Complete model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=BC, id=ID)
summary(coxPH_model8)
```

Complete model + dx_yr
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=BC, id=ID)
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=BC)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=BC)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=BC)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model4 <- lm(fvc_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=BC)
summary(FVC_model4)
confint(FVC_model4)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=BC)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=BC)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=BC)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=BC)
summary(DLCO_model3)
confint(DLCO_model3)
```


# IPF-Only - Cox PH Models for Association between BC and Death or Lung Transplant
## IPF-Only - BC 5yrs Pre-Censoring Models
### IPF-Only - Continuous BC 5yrs pre-censoring until Death/Transplant/Censoring
Start with the simplest model just looking at BC in the year of and 5yrs prior to death/transplant/censoring
```{r}
coxPH_model1 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC_5yrPreCensor, data=BC_IPF, id=ID)
summary(coxPH_model1)
```

Complete model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=BC_IPF, id=ID)
summary(coxPH_model2)
```

Complete model + dx_yr
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=BC_IPF, id=ID)
summary(coxPH_model3)
```
...
### IPF-Only - Variable HR for BC risk in 5yr Pre-Cenosr
Want a plot with x-axis of BC level and y-axis of HR

Unadjusted model
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
BCx <- BC_IPF %>% filter(!is.na(BC_5yrPreCensor) & !is.na(time_DeathTxCensor))
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
BCx <- BC_IPF %>% filter(!is.na(BC_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat))
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
BCx <- BC_IPF %>% filter(!is.na(BC_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat) & !is.na(dx_yr))
...
### IPF-Only - BC Low vs High 5yrs Pre-Censoring
Base Model:
```{r}
coxPH_model7 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich, data=BC_IPF, id=ID)
summary(coxPH_model7)
```

Complete model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=BC_IPF, id=ID)
summary(coxPH_model8)
```

Complete model + dx_yr
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=BC_IPF, id=ID)
summary(coxPH_model9)
```
...
Now I will make a Kaplan-Meier survival curve, discretized by BC during disease deciles:
```{r, fig.height=5, fig.width=7}
surv1 <- survfit(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich, data=BC_IPF)
(ggsurvplot(surv1, 
            palette=c("dodgerblue4","firebrick4"),
            censor=TRUE,
            pval=TRUE,
...
# IPF-Only - Association between BC and Baseline FVC
...
## IPF-Only - BC 5yrs Pre-Diagnosis and FVC Models
Looking at continuous BC_5yrPreDx vs baseline FVC

Simplest model just looking at BC in 5yrs pre-diagnosis
```{r}
FVC_model1 <- lm(fvc_pct ~ BC_5yrPreDx, data=BC_IPF)
summary(FVC_model1)
confint(FVC_model1)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=BC_IPF)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=BC_IPF)
summary(FVC_model3)
confint(FVC_model3)
```


### IPF-Only - BC 5yrs Pre-Diagnosis Low vs High and Baseline FVC
Looking at BC5yr_dich vs baseline FVC

Simplest model just looking at BC in 5yrs pre-diagnosis
```{r}
FVC_model1 <- lm(fvc_pct ~ BC5yr_dich, data=BC_IPF)
summary(FVC_model1)
confint(FVC_model1)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=BC_IPF)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=BC_IPF)
summary(FVC_model3)
confint(FVC_model3)
```


# IPF-Only - Association between BC and Baseline DLCO
## IPF-Only - Visual EDA 5yrs pre-Dx
Scatterplot of continuous BC 5yrs pre-diagnosis vs DLCO, with sex represented by the color of points
```{r}
(BC %>% ggplot(aes(x=BC_5yrPreDx, y=dlco_pct, color=sex))+
...
## IPF-Only - BC 5yrs Pre-Diagnosis and DLCO Models
### IPF-Only - BC 5yrs Pre-Diagnosis and Baseline DLCO
Looking at continuous BC_5yrPreDx vs baseline DLCO

Simplest model just looking at BC in 5yrs pre-diagnosis
```{r}
DLCO_model1 <- lm(dlco_pct ~ BC_5yrPreDx, data=BC_IPF)
summary(DLCO_model1)
confint(DLCO_model1)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=BC_IPF)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=BC_IPF)
summary(DLCO_model3)
confint(DLCO_model3)
```

### IPF-Only - BC 5yrs Pre-Diagnosis Low vs High and Baseline DLCO
Looking at BC5yr_dich vs baseline DLCO

Simplest model just looking at BC in 5yrs pre-diagnosis
```{r}
DLCO_model1 <- lm(dlco_pct ~ BC5yr_dich, data=BC_IPF)
summary(DLCO_model1)
confint(DLCO_model1)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=BC_IPF)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=BC_IPF)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
```{r}
pre2015 <- BC %>% filter(dx_yr<=2015)

post2015 <- BC %>% filter(dx_yr>2015)
```

# Pre/Post-2015 Only - Cox PH Models for Association between BC and Death or Lung Transplant
## Pre/Post-2015 Only - BC 5yrs Pre-Censoring Models
### Pre/Post-2015 Only - Continuous BC 5yrs pre-censoring until Death/Transplant/Censoring
Start with the simplest model just looking at BC in the year of and 5yrs prior to death/transplant/censoring
```{r}
coxPH_model1a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC_5yrPreCensor, data=pre2015, id=ID)
summary(coxPH_model1a)
```

```{r}
coxPH_model1b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC_5yrPreCensor, data=post2015, id=ID)
summary(coxPH_model1b)
```

Complete model
```{r}
coxPH_model2a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015, id=ID)
summary(coxPH_model2a)
```

Complete model
```{r}
coxPH_model2b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015, id=ID)
summary(coxPH_model2b)
```

### Pre/Post-2015 Only - Variable HR for BC risk in 5yr Pre-Cenosr
Want a plot with x-axis of BC level and y-axis of HR

Unadjusted model - Pre-2015
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
pre2015x <- pre2015 %>% filter(!is.na(BC_5yrPreCensor) & !is.na(time_DeathTxCensor))

#Then make survival function
surv1 <- Surv(pre2015x$time_DeathTxCensor, pre2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(pre2015x$BC_5yrPreCensor, df=3))
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)

#Then plot
plot(pre2015x$BC_5yrPreCensor, exp(predicted$fit), type="n", xlim=c(0,1.75), ylim=c(0,5))
lines(sm.spline(pre2015x$BC_5yrPreCensor, exp(predicted$fit)), col = "red" , lty = 1 )
lines(sm.spline(pre2015x$BC_5yrPreCensor, exp(predicted$fit + 1.96 * predicted$se)), col = "orange" , lty = 2 )
lines(sm.spline(pre2015x$BC_5yrPreCensor, exp(predicted$fit - 1.96 * predicted$se)), col = "orange" , lty = 2 )
```
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
post2015x <- post2015 %>% filter(!is.na(BC_5yrPreCensor) & !is.na(time_DeathTxCensor))

#Then make survival function
surv1 <- Surv(post2015x$time_DeathTxCensor, post2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(post2015x$BC_5yrPreCensor, df=3))
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)

#Then plot
plot(post2015x$BC_5yrPreCensor, exp(predicted$fit), type="n", xlim=c(0,1.75), ylim=c(0,5))
lines(sm.spline(post2015x$BC_5yrPreCensor, exp(predicted$fit)), col = "red" , lty = 1 )
lines(sm.spline(post2015x$BC_5yrPreCensor, exp(predicted$fit + 1.96 * predicted$se)), col = "orange" , lty = 2 )
lines(sm.spline(post2015x$BC_5yrPreCensor, exp(predicted$fit - 1.96 * predicted$se)), col = "orange" , lty = 2 )
```
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
pre2015x <- pre2015 %>% filter(!is.na(BC_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat))

#Then survival function
surv1 <- Surv(pre2015x$time_DeathTxCensor, pre2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(pre2015x$BC_5yrPreCensor, df=3) + pre2015x$age_dx + pre2015x$sex + pre2015x$smokeHx + pre2015x$dich_Race + pre2015x$ADI_nat)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
post2015x <- post2015 %>% filter(!is.na(BC_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat))

#Then survival function
surv1 <- Surv(post2015x$time_DeathTxCensor, post2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(post2015x$BC_5yrPreCensor, df=3) + post2015x$age_dx + post2015x$sex + post2015x$smokeHx + post2015x$dich_Race + post2015x$ADI_nat)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
### Pre/Post-2015 Only - BC Low vs High 5yrs Pre-Censoring
Base Model:
```{r}
coxPH_model7a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich, data=pre2015, id=ID)
summary(coxPH_model7a)
```

Base Model:
```{r}
coxPH_model7b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich, data=post2015, id=ID)
summary(coxPH_model7b)
```

Complete model
```{r}
coxPH_model8a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015, id=ID)
summary(coxPH_model8a)
```

Complete model
```{r}
coxPH_model8b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015, id=ID)
...
Now I will make a Kaplan-Meier survival curve, discretized by BC during disease deciles:
```{r, fig.height=5, fig.width=7}
surv1 <- survfit(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich, data=pre2015)
(ggsurvplot(surv1, 
            palette=c("dodgerblue4","firebrick4"),
            censor=TRUE,
            pval=TRUE,
...
Now I will make a Kaplan-Meier survival curve, discretized by BC during disease deciles:
```{r, fig.height=5, fig.width=7}
surv1 <- survfit(Surv(time_DeathTxCensor, deadORtx==1) ~ BC5yrCensor_dich, data=post2015)
(ggsurvplot(surv1, 
            palette=c("dodgerblue4","firebrick4"),
            censor=TRUE,
            pval=TRUE,
...
# Pre/Post-2015 Only - Association between BC and Baseline FVC
## Pre/Post-2015 Only - Visual EDA 5yrs pre-Dx
Scatterplot of continuous BC 5yrs pre-diagnosis vs FVC, with sex represented by the color of points - Pre-2015
```{r}
(pre2015 %>% ggplot(aes(x=BC_5yrPreDx, y=fvc_pct, color=sex))+
   geom_point()+
   labs(x="BC 5yrs Pre-Diagnosis", y="Baseline FVC % Predicted", title="Baseline FVC % Predicted by BC 5yrs Pre-Diagnosis")+
   geom_smooth(method="lm", se = FALSE) +
   theme(plot.title = element_text(hjust = 0.5))+
...
Scatterplot of continuous BC 5yrs pre-diagnosis vs FVC, with sex represented by the color of points - Post-2015
```{r}
(post2015 %>% ggplot(aes(x=BC_5yrPreDx, y=fvc_pct, color=sex))+
   geom_point()+
   labs(x="BC 5yrs Pre-Diagnosis", y="Baseline FVC % Predicted", title="Baseline FVC % Predicted by BC 5yrs Pre-Diagnosis")+
   geom_smooth(method="lm", se = FALSE) +
   theme(plot.title = element_text(hjust = 0.5))+
...
Violin plot wrapping boxplot to visualize BC 5yrs pre-dx low vs high vs FVC - Pre-2015
```{r}
pre2015b <- pre2015 %>% filter(!is.na(BC5yr_dich))
(pre2015b %>% ggplot(aes(x=BC5yr_dich, y=fvc_pct, fill=BC5yr_dich))+
   geom_boxplot(width=0.2, color="black", alpha=1.0)+
   geom_violin(width=1.0, alpha=0.5)+
   labs(x="BC 5yrs Pre-Diagnosis Low vs High", y="Baseline FVC % Predicted", title="Baseline FVC % Predicted by BC 5yrs Pre-Diagnosis Low vs High")+
   theme_light()+
...
Violin plot wrapping boxplot to visualize BC 5yrs pre-dx low vs high vs FVC - Post-2015
```{r}
post2015b <- post2015 %>% filter(!is.na(BC5yr_dich))
(post2015b %>% ggplot(aes(x=BC5yr_dich, y=fvc_pct, fill=BC5yr_dich))+
   geom_boxplot(width=0.2, color="black", alpha=1.0)+
   geom_violin(width=1.0, alpha=0.5)+
   labs(x="BC 5yrs Pre-Diagnosis Low vs High", y="Baseline FVC % Predicted", title="Baseline FVC % Predicted by BC 5yrs Pre-Diagnosis Low vs High")+
   theme_light()+
...
## Pre/Post-2015 Only - BC 5yrs Pre-Diagnosis and FVC Models
Looking at continuous BC_5yrPreDx vs baseline FVC

Simplest model just looking at BC in 5yrs pre-diagnosis
```{r}
FVC_model1a <- lm(fvc_pct ~ BC_5yrPreDx, data=pre2015)
summary(FVC_model1a)
confint(FVC_model1a)
```
...
Simplest model just looking at BC in 5yrs pre-diagnosis
```{r}
FVC_model1b <- lm(fvc_pct ~ BC_5yrPreDx, data=post2015)
summary(FVC_model1b)
confint(FVC_model1b)
```
...
Complete model
```{r}
FVC_model2a <- lm(fvc_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015)
summary(FVC_model2a)
confint(FVC_model2a)
```
...
Complete model
```{r}
FVC_model2b <- lm(fvc_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015)
summary(FVC_model2b)
confint(FVC_model2b)
```


### Pre/Post-2015 Only - BC 5yrs Pre-Diagnosis Low vs High and Baseline FVC
Looking at BC5yr_dich vs baseline FVC

Simplest model just looking at BC in 5yrs pre-diagnosis
```{r}
FVC_model1a <- lm(fvc_pct ~ BC5yr_dich, data=pre2015)
summary(FVC_model1a)
confint(FVC_model1a)
```
...
Simplest model just looking at BC in 5yrs pre-diagnosis
```{r}
FVC_model1b <- lm(fvc_pct ~ BC5yr_dich, data=post2015)
summary(FVC_model1b)
confint(FVC_model1b)
```
...
Complete model
```{r}
FVC_model2a <- lm(fvc_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015)
summary(FVC_model2a)
confint(FVC_model2a)
```
...
Complete model
```{r}
FVC_model2b <- lm(fvc_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015)
summary(FVC_model2b)
confint(FVC_model2b)
```

# Pre/Post-2015 Only - Association between BC and Baseline DLCO
## Pre/Post-2015 Only - Visual EDA 5yrs pre-Dx
Scatterplot of continuous BC 5yrs pre-diagnosis vs DLCO, with sex represented by the color of points - Pre-2015
```{r}
(pre2015 %>% ggplot(aes(x=BC_5yrPreDx, y=dlco_pct, color=sex))+
   geom_point()+
   labs(x="BC 5yrs Pre-Diagnosis", y="Baseline DLCO % Predicted", title="Baseline DLCO % Predicted by BC 5yrs Pre-Diagnosis")+
   geom_smooth(method="lm", se = FALSE) +
   theme(plot.title = element_text(hjust = 0.5))+
...
Scatterplot of continuous BC 5yrs pre-diagnosis vs DLCO, with sex represented by the color of points - Post-2015
```{r}
(post2015 %>% ggplot(aes(x=BC_5yrPreDx, y=dlco_pct, color=sex))+
   geom_point()+
   labs(x="BC 5yrs Pre-Diagnosis", y="Baseline DLCO % Predicted", title="Baseline DLCO % Predicted by BC 5yrs Pre-Diagnosis")+
   geom_smooth(method="lm", se = FALSE) +
   theme(plot.title = element_text(hjust = 0.5))+
...
Violin plot wrapping boxplot to visualize BC 5yrs pre-dx low vs high vs DLCO - Pre-2015
```{r}
(pre2015b %>% ggplot(aes(x=BC5yr_dich, y=dlco_pct, fill=BC5yr_dich))+
   geom_boxplot(width=0.2, color="black", alpha=1.0)+
   geom_violin(width=1.0, alpha=0.5)+
   labs(x="BC 5yrs Pre-Diagnosis Low vs High", y="Baseline DLCO % Predicted", title="Baseline DLCO % Predicted by BC 5yrs Pre-Diagnosis Low vs High")+
   theme_light()+
...
Violin plot wrapping boxplot to visualize BC 5yrs pre-dx low vs high vs DLCO - Post-2015
```{r}
(post2015b %>% ggplot(aes(x=BC5yr_dich, y=dlco_pct, fill=BC5yr_dich))+
   geom_boxplot(width=0.2, color="black", alpha=1.0)+
   geom_violin(width=1.0, alpha=0.5)+
   labs(x="BC 5yrs Pre-Diagnosis Low vs High", y="Baseline DLCO % Predicted", title="Baseline DLCO % Predicted by BC 5yrs Pre-Diagnosis Low vs High")+
   theme_light()+
...
## Pre/Post-2015 Only - BC 5yrs Pre-Diagnosis and DLCO Models
### Pre/Post-2015 Only - BC 5yrs Pre-Diagnosis and Baseline DLCO
Looking at continuous BC_5yrPreDx vs baseline DLCO

Simplest model just looking at BC in 5yrs pre-diagnosis
```{r}
DLCO_model1a <- lm(dlco_pct ~ BC_5yrPreDx, data=pre2015)
...
Simplest model just looking at BC in 5yrs pre-diagnosis
```{r}
DLCO_model1b <- lm(dlco_pct ~ BC_5yrPreDx, data=post2015)
...
Complete model
```{r}
DLCO_model2a <- lm(dlco_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015)
...
Complete model
```{r}
DLCO_model2b <- lm(dlco_pct ~ BC_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015)
summary(DLCO_model2b)
confint(DLCO_model2b)
```

### Pre/Post-2015 Only - BC 5yrs Pre-Diagnosis Low vs High and Baseline DLCO
Looking at BC5yr_dich vs baseline DLCO

Simplest model just looking at BC in 5yrs pre-diagnosis
```{r}
DLCO_model1a <- lm(dlco_pct ~ BC5yr_dich, data=pre2015)
summary(DLCO_model1a)
confint(DLCO_model1a)
```
...
Simplest model just looking at BC in 5yrs pre-diagnosis
```{r}
DLCO_model1b <- lm(dlco_pct ~ BC5yr_dich, data=post2015)
summary(DLCO_model1b)
confint(DLCO_model1b)
```
...
Complete model
```{r}
DLCO_model2a <- lm(dlco_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015)
summary(DLCO_model2a)
confint(DLCO_model2a)
```
...
Complete model
```{r}
DLCO_model2b <- lm(dlco_pct ~ BC5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015)
summary(DLCO_model2b)
confint(DLCO_model2b)
```
