---
title: "Monthly Satellite NO3 Analysis with Simmons Data - Revision #1 Final Analysis"
...
```{r}
outfile2 <- here("MonthlyNO3Data/Simmons_fILDPts_BaselineData_2022_03_28.xlsx")
Simm <- read_excel(outfile2)
```

# Merging Datasets
I used a inner_join here so that the complete "NO3" dataframe only includes patients with fILD that have all baseline demographics and ADI available as well as NO3 data.
```{r}
NO3 <- inner_join(Simm, NO3, by="ID")
```
...
```{r}
NO3 <- NO3 %>% 
  pivot_longer(cols=c(49:264), names_to="NO3_date", names_prefix="NIT_")
```
...
```{r}
NO3$smokeHx <- as.character(NO3$smokeHx) 
NO3 <- NO3 %>% mutate(smokeHx1=if_else(is.na(smokeHx), "Unknown", smokeHx))

#now need to make new dich_smoking category
NO3$dich_smoking <- as.character(NO3$dich_smoking) 
NO3 <- NO3 %>% mutate(dich_smoking1=if_else(is.na(dich_smoking),"Unknown", dich_smoking))
```

Now need to remove old smoking variables and rename new ones
```{r}
NO3 <- NO3 %>% dplyr::select(-c(smokeHx, dich_smoking))
NO3 <- NO3 %>% rename(c("smokeHx"="smokeHx1", "dich_smoking"="dich_smoking1"))
NO3$smokeHx <- as.factor(NO3$smokeHx)
NO3$dich_smoking <- as.factor(NO3$dich_smoking)
NO3$smokeHx <- fct_relevel(NO3$smokeHx, c("Never","Former","Always","Unknown"))
NO3$dich_smoking <- fct_relevel(NO3$dich_smoking, c("Never","Ever","Unknown"))
```
...
NO3$metro <- fct_relevel(NO3$metro, c("metropolitan", "micropolitan", "rural"))
...
```{r}
write_xlsx(NO3, path="Final_Simmons_fILD_NO3_BaselineData_2022_03_28.xlsx")
```
...
Violin plot wrapping boxplot to visualize low vs high NO35yrCensor_dich vs ADI
```{r}
(NO3 %>% ggplot(aes(x=NO35yrCensor_dich, y=ADI_nat, fill=NO35yrCensor_dich))+
   geom_boxplot(width=0.2, color="black", alpha=1.0)+
   geom_violin(width=1.0, alpha=0.5)+
   labs(x="Low vs High NO3 in 5yrs Pre-Censoring", y="National ADI", title="National ADI by NO3 Exposure")+
   theme(plot.title = element_text(hjust = 0.5))+
   ylim(0,100))
```
...
Complete model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=NO3, id=ID)
summary(coxPH_model2)
```

Complete model + dx_yr
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=NO3, id=ID)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
NO3x <- NO3 %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat))

#Then survival function
surv1 <- Surv(NO3x$time_DeathTxCensor, NO3x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(NO3x$NO3_5yrPreCensor, df=3) + NO3x$age_dx + NO3x$sex + NO3x$smokeHx + NO3x$dich_Race + NO3x$ADI_nat)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
NO3x <- NO3 %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat) &!is.na(dx_yr))

#Then survival function
surv1 <- Surv(NO3x$time_DeathTxCensor, NO3x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(NO3x$NO3_5yrPreCensor, df=3) + NO3x$age_dx + NO3x$sex + NO3x$smokeHx + NO3x$dich_Race + NO3x$ADI_nat + NO3x$dx_yr)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Complete model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=NO3, id=ID)
summary(coxPH_model8)
```

Complete model + dx_yr
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=NO3, id=ID)
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=NO3)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=NO3)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=NO3)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model4 <- lm(fvc_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=NO3)
summary(FVC_model4)
confint(FVC_model4)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=NO3)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=NO3)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=NO3)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=NO3)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=NO3_IPF, id=ID)
summary(coxPH_model2)
```

Complete model + dx_yr
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=NO3_IPF, id=ID)
summary(coxPH_model3)
```
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
NO3x <- NO3_IPF %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat))
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
NO3x <- NO3_IPF %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat) & !is.na(dx_yr))
...
Complete model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=NO3_IPF, id=ID)
summary(coxPH_model8)
```

Complete model + dx_yr
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=NO3_IPF, id=ID)
summary(coxPH_model9)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=NO3_IPF)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=NO3_IPF)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=NO3_IPF)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=NO3_IPF)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=NO3_IPF)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=NO3_IPF)
summary(DLCO_model3)
confint(DLCO_model3)
```

### IPF-Only - NO3 5yrs Pre-Diagnosis Low vs High and Baseline DLCO
Looking at NO35yr_dich vs baseline DLCO
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=NO3_IPF)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=NO3_IPF)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
coxPH_model2a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015, id=ID)
summary(coxPH_model2a)
```

Complete model
```{r}
coxPH_model2b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015, id=ID)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
pre2015x <- pre2015 %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat))

#Then survival function
surv1 <- Surv(pre2015x$time_DeathTxCensor, pre2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(pre2015x$NO3_5yrPreCensor, df=3) + pre2015x$age_dx + pre2015x$sex + pre2015x$smokeHx + pre2015x$dich_Race + pre2015x$ADI_nat)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
post2015x <- post2015 %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat))

#Then survival function
surv1 <- Surv(post2015x$time_DeathTxCensor, post2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(post2015x$NO3_5yrPreCensor, df=3) + post2015x$age_dx + post2015x$sex + post2015x$smokeHx + post2015x$dich_Race + post2015x$ADI_nat)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Complete model
```{r}
coxPH_model8a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015, id=ID)
summary(coxPH_model8a)
```

Complete model
```{r}
coxPH_model8b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015, id=ID)
...
Complete model
```{r}
FVC_model2a <- lm(fvc_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015)
summary(FVC_model2a)
confint(FVC_model2a)
```
...
Complete model
```{r}
FVC_model2b <- lm(fvc_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015)
summary(FVC_model2b)
confint(FVC_model2b)
```
...
Complete model
```{r}
FVC_model2a <- lm(fvc_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015)
summary(FVC_model2a)
confint(FVC_model2a)
```
...
Complete model
```{r}
FVC_model2b <- lm(fvc_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015)
summary(FVC_model2b)
confint(FVC_model2b)
```
...
Complete model
```{r}
DLCO_model2a <- lm(dlco_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015)
...
Complete model
```{r}
DLCO_model2b <- lm(dlco_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015)
...
Complete model
```{r}
DLCO_model2a <- lm(dlco_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015)
summary(DLCO_model2a)
confint(DLCO_model2a)
```
...
Complete model
```{r}
DLCO_model2b <- lm(dlco_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015)
summary(DLCO_model2b)
confint(DLCO_model2b)
```
