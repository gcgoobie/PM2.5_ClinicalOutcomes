---
title: "Monthly Satellite OM Analysis with Simmons Data - Revision #1 Final Analysis"
...
```{r}
outfile1 <- here("MonthlyOMData/Simm_fILD_2000_2017_OM_2021_11_05.xlsx")
...
```{r}
outfile2 <- here("MonthlyOMData/Simmons_fILDPts_BaselineData_2022_03_28.xlsx")
Simm <- read_excel(outfile2)
```

# Merging Datasets
I used a inner_join here so that the complete "OM" dataframe only includes patients with fILD that have all baseline demographics and ADI available as well as OM data.
```{r}
OM <- inner_join(Simm, OM, by="ID")
```
...
```{r}
OM <- OM %>% 
  pivot_longer(cols=c(49:264), names_to="OM_date", names_prefix="OM_")
```
...
```{r}
OM$smokeHx <- as.character(OM$smokeHx) 
OM <- OM %>% mutate(smokeHx1=if_else(is.na(smokeHx), "Unknown", smokeHx))

#now need to make new dich_smoking category
OM$dich_smoking <- as.character(OM$dich_smoking) 
OM <- OM %>% mutate(dich_smoking1=if_else(is.na(dich_smoking),"Unknown", dich_smoking))
```

Now need to remove old smoking variables and rename new ones
```{r}
OM <- OM %>% dplyr::select(-c(smokeHx, dich_smoking))
OM <- OM %>% rename(c("smokeHx"="smokeHx1", "dich_smoking"="dich_smoking1"))
OM$smokeHx <- as.factor(OM$smokeHx)
OM$dich_smoking <- as.factor(OM$dich_smoking)
OM$smokeHx <- fct_relevel(OM$smokeHx, c("Never","Former","Always","Unknown"))
OM$dich_smoking <- fct_relevel(OM$dich_smoking, c("Never","Ever","Unknown"))
```
...
OM$metro <- fct_relevel(OM$metro, c("metropolitan", "micropolitan", "rural"))
...
```{r}
write_xlsx(OM, path="Final_Simmons_fILD_OM_BaselineData_2022_03_28.xlsx")
```
...
Violin plot wrapping boxplot to visualize low vs high OM5yrCensor_dich vs ADI
```{r}
(OM %>% ggplot(aes(x=OM5yrCensor_dich, y=ADI_nat, fill=OM5yrCensor_dich))+
   geom_boxplot(width=0.2, color="black", alpha=1.0)+
   geom_violin(width=1.0, alpha=0.5)+
   labs(x="Low vs High OM in 5yrs Pre-Censoring", y="National ADI", title="National ADI by OM Exposure")+
   theme(plot.title = element_text(hjust = 0.5))+
   ylim(0,100))
```
...
Complete model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ OM_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=OM, id=ID)
summary(coxPH_model2)
```

Complete model + dx_yr
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ OM_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=OM, id=ID)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
OMx <- OM %>% filter(!is.na(OM_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat))

#Then survival function
surv1 <- Surv(OMx$time_DeathTxCensor, OMx$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(OMx$OM_5yrPreCensor, df=3) + OMx$age_dx + OMx$sex + OMx$smokeHx + OMx$dich_Race + OMx$ADI_nat)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
OMx <- OM %>% filter(!is.na(OM_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat) &!is.na(dx_yr))

#Then survival function
surv1 <- Surv(OMx$time_DeathTxCensor, OMx$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(OMx$OM_5yrPreCensor, df=3) + OMx$age_dx + OMx$sex + OMx$smokeHx + OMx$dich_Race + OMx$ADI_nat + OMx$dx_yr)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Complete model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ OM5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=OM, id=ID)
summary(coxPH_model8)
```

Complete model + dx_yr
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ OM5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=OM, id=ID)
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ OM_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=OM)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ OM_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=OM)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ OM5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=OM)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model4 <- lm(fvc_pct ~ OM5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=OM)
summary(FVC_model4)
confint(FVC_model4)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ OM_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=OM)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ OM_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=OM)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ OM5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=OM)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ OM5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=OM)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ OM_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=OM_IPF, id=ID)
summary(coxPH_model2)
```

Complete model + dx_yr
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ OM_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=OM_IPF, id=ID)
summary(coxPH_model3)
```
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
OMx <- OM_IPF %>% filter(!is.na(OM_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat))
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
OMx <- OM_IPF %>% filter(!is.na(OM_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat) & !is.na(dx_yr))
...
Complete model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ OM5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=OM_IPF, id=ID)
summary(coxPH_model8)
```

Complete model + dx_yr
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ OM5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=OM_IPF, id=ID)
summary(coxPH_model9)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ OM_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=OM_IPF)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ OM_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=OM_IPF)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model2 <- lm(fvc_pct ~ OM5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=OM_IPF)
summary(FVC_model2)
confint(FVC_model2)
```
...
Complete model + dx_yr
```{r}
FVC_model3 <- lm(fvc_pct ~ OM5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=OM_IPF)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ OM_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=OM_IPF)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ OM_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=OM_IPF)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
DLCO_model2 <- lm(dlco_pct ~ OM5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=OM_IPF)
summary(DLCO_model2)
confint(DLCO_model2)
```
...
Complete model + dx_yr
```{r}
DLCO_model3 <- lm(dlco_pct ~ OM5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_yr, data=OM_IPF)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
coxPH_model2a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ OM_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015, id=ID)
summary(coxPH_model2a)
```

Complete model
```{r}
coxPH_model2b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ OM_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015, id=ID)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
pre2015x <- pre2015 %>% filter(!is.na(OM_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat))

#Then survival function
surv1 <- Surv(pre2015x$time_DeathTxCensor, pre2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(pre2015x$OM_5yrPreCensor, df=3) + pre2015x$age_dx + pre2015x$sex + pre2015x$smokeHx + pre2015x$dich_Race + pre2015x$ADI_nat)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
post2015x <- post2015 %>% filter(!is.na(OM_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(ADI_nat))

#Then survival function
surv1 <- Surv(post2015x$time_DeathTxCensor, post2015x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(post2015x$OM_5yrPreCensor, df=3) + post2015x$age_dx + post2015x$sex + post2015x$smokeHx + post2015x$dich_Race + post2015x$ADI_nat)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Complete model
```{r}
coxPH_model8a <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ OM5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015, id=ID)
summary(coxPH_model8a)
```

Complete model
```{r}
coxPH_model8b <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ OM5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015, id=ID)
...
Complete model
```{r}
FVC_model2a <- lm(fvc_pct ~ OM_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015)
summary(FVC_model2a)
confint(FVC_model2a)
```
...
Complete model
```{r}
FVC_model2b <- lm(fvc_pct ~ OM_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015)
summary(FVC_model2b)
confint(FVC_model2b)
```
...
Complete model
```{r}
FVC_model2a <- lm(fvc_pct ~ OM5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015)
summary(FVC_model2a)
confint(FVC_model2a)
```
...
Complete model
```{r}
FVC_model2b <- lm(fvc_pct ~ OM5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015)
summary(FVC_model2b)
confint(FVC_model2b)
```
...
Complete model
```{r}
DLCO_model2a <- lm(dlco_pct ~ OM_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015)
...
Complete model
```{r}
DLCO_model2b <- lm(dlco_pct ~ OM_5yrPreDx + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015)
...
Complete model
```{r}
DLCO_model2a <- lm(dlco_pct ~ OM5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=pre2015)
summary(DLCO_model2a)
confint(DLCO_model2a)
```
...
Complete model
```{r}
DLCO_model2b <- lm(dlco_pct ~ OM5yr_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat, data=post2015)
summary(DLCO_model2b)
confint(DLCO_model2b)
```
