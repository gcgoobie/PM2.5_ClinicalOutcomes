---
title: "Monthly Satellite NO3 Analysis with Simmons Data - Updated First Encounter Date"
...
```{r}
write_xlsx(NO3, path="Final_Simmons_fILD_NO3_BaselineData_2022_03_29.xlsx")
```
...
```{r}
(NO3 %>% ggplot(aes(x=NO35yrCensor_dich, y=ADI_nat, fill=NO35yr_dich))+
   geom_boxplot(width=0.2, color="black", alpha=1.0)+
   geom_violin(width=1.0, alpha=0.5)+
   labs(x="Low vs High NO3 in 5yrs Pre-Diagnosis", y="National ADI", title="National ADI by NO3 Exposure")+
   theme(plot.title = element_text(hjust = 0.5))+
   ylim(0,100))
```
...
Partial model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + dx_IPF + ADI_nat + metro, data=NO3, id=ID)
...
Complete model
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO3_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + dx_IPF + ADI_nat + metro + fvc_pct + dlco_pct, data=NO3, id=ID)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
NO3x <- NO3 %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(dx_IPF) & !is.na(ADI_nat) & !is.na(metro))

#Then survival function
surv1 <- Surv(NO3x$time_DeathTxCensor, NO3x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(NO3x$NO3_5yrPreCensor, df=4) + NO3x$age_dx + NO3x$sex + NO3x$smokeHx + NO3x$dich_Race + NO3x$dx_IPF + NO3x$ADI_nat + NO3x$metro)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
NO3x <- NO3 %>% filter(!is.na(NO3_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(dx_IPF) & !is.na(ADI_nat) & !is.na(metro) & !is.na(fvc_pct) & !is.na(dlco_pct))

#Then survival function
surv1 <- Surv(NO3x$time_DeathTxCensor, NO3x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(NO3x$NO3_5yrPreCensor, df=4) + NO3x$age_dx + NO3x$sex + NO3x$smokeHx + NO3x$dich_Race + NO3x$dx_IPF + NO3x$ADI_nat + NO3x$metro + NO3x$fvc_pct + NO3x$dlco_pct)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Partial model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_IPF + metro, data=NO3, id=ID)
...
Complete model
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NO35yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_IPF + metro + fvc_pct + dlco_pct, data=NO3, id=ID)
...
Complete model
```{r}
FVC_model3 <- lm(fvc_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + dx_IPF + ADI_nat + metro, data=NO3)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model3 <- lm(fvc_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + dx_IPF + ADI_nat + metro, data=NO3)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
DLCO_model3 <- lm(dlco_pct ~ NO3_5yrPreDx + sex + age_dx + smokeHx + dich_Race + dx_IPF + ADI_nat + metro, data=NO3)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
DLCO_model3 <- lm(dlco_pct ~ NO35yr_dich + sex + age_dx + smokeHx + dich_Race + dx_IPF + ADI_nat + metro, data=NO3)
summary(DLCO_model3)
confint(DLCO_model3)
```
