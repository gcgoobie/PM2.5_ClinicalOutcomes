---
title: "Monthly Satellite PM Analysis with CARE-PF Data - Updated Dichotomized PM"
...
Partial model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ PM_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro, data=PM, id=ID)
...
Complete model
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ PM_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro + fvc_pct + dlco_pct, data=PM, id=ID)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
PMx <- PM %>% filter(!is.na(PM_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(dx_IPF) & !is.na(avg_s) & !is.na(metro))

#Then survival function
surv1 <- Surv(PMx$time_DeathTxCensor, PMx$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(PMx$PM_5yrPreCensor, df=4) + PMx$age_dx + PMx$sex + PMx$smokeHx + PMx$dich_Race + PMx$dx_IPF + PMx$avg_s + PMx$metro)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
PMx <- PM %>% filter(!is.na(PM_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(dx_IPF) & !is.na(avg_s) & !is.na(metro) & !is.na(fvc_pct) & !is.na(dlco_pct))

#Then survival function
surv1 <- Surv(PMx$time_DeathTxCensor, PMx$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(PMx$PM_5yrPreCensor, df=4) + PMx$age_dx + PMx$sex + PMx$smokeHx + PMx$dich_Race + PMx$dx_IPF + PMx$avg_s + PMx$metro + PMx$fvc_pct + PMx$dlco_pct)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Partial model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ PM5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + dx_IPF + metro, data=PM, id=ID)
...
Complete model
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ PM5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + dx_IPF + metro + fvc_pct + dlco_pct, data=PM, id=ID)
...
Complete model
```{r}
FVC_model3 <- lm(fvc_pct ~ PM_5yrPreDx + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro, data=PM)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model3 <- lm(fvc_pct ~ PM5yr_dich + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro, data=PM)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
DLCO_model3 <- lm(dlco_pct ~ PM_5yrPreDx + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro, data=PM)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
DLCO_model3 <- lm(dlco_pct ~ PM5yr_dich + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro, data=PM)
summary(DLCO_model3)
confint(DLCO_model3)
```
