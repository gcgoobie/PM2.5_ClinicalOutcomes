---
title: "Monthly Satellite SS Analysis with Simmons Data - Updated First Encounter Date"
...
```{r}
write_xlsx(SS, path="Final_Simmons_fILD_SS_BaselineData_2022_03_29.xlsx")
```
...
```{r}
(SS %>% ggplot(aes(x=SS5yrCensor_dich, y=ADI_nat, fill=SS5yr_dich))+
   geom_boxplot(width=0.2, color="black", alpha=1.0)+
   geom_violin(width=1.0, alpha=0.5)+
   labs(x="Low vs High SS in 5yrs Pre-Diagnosis", y="National ADI", title="National ADI by SS Exposure")+
   theme(plot.title = element_text(hjust = 0.5))+
   ylim(0,100))
```
...
Partial model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SS_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + dx_IPF + ADI_nat + metro, data=SS, id=ID)
...
Complete model
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SS_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + dx_IPF + ADI_nat + metro + fvc_pct + dlco_pct, data=SS, id=ID)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
SSx <- SS %>% filter(!is.na(SS_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(dx_IPF) & !is.na(ADI_nat) & !is.na(metro))

#Then survival function
surv1 <- Surv(SSx$time_DeathTxCensor, SSx$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(SSx$SS_5yrPreCensor, df=4) + SSx$age_dx + SSx$sex + SSx$smokeHx + SSx$dich_Race + SSx$dx_IPF + SSx$ADI_nat + SSx$metro)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
SSx <- SS %>% filter(!is.na(SS_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(dx_IPF) & !is.na(ADI_nat) & !is.na(metro) & !is.na(fvc_pct) & !is.na(dlco_pct))

#Then survival function
surv1 <- Surv(SSx$time_DeathTxCensor, SSx$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(SSx$SS_5yrPreCensor, df=4) + SSx$age_dx + SSx$sex + SSx$smokeHx + SSx$dich_Race + SSx$dx_IPF + SSx$ADI_nat + SSx$metro + SSx$fvc_pct + SSx$dlco_pct)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Partial model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SS5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_IPF + metro, data=SS, id=ID)
...
Complete model
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ SS5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + ADI_nat + dx_IPF + metro + fvc_pct + dlco_pct, data=SS, id=ID)
...
Complete model
```{r}
FVC_model3 <- lm(fvc_pct ~ SS_5yrPreDx + sex + age_dx + smokeHx + dich_Race + dx_IPF + ADI_nat + metro, data=SS)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model3 <- lm(fvc_pct ~ SS5yr_dich + sex + age_dx + smokeHx + dich_Race + dx_IPF + ADI_nat + metro, data=SS)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
DLCO_model3 <- lm(dlco_pct ~ SS_5yrPreDx + sex + age_dx + smokeHx + dich_Race + dx_IPF + ADI_nat + metro, data=SS)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
DLCO_model3 <- lm(dlco_pct ~ SS5yr_dich + sex + age_dx + smokeHx + dich_Race + dx_IPF + ADI_nat + metro, data=SS)
summary(DLCO_model3)
confint(DLCO_model3)
```
