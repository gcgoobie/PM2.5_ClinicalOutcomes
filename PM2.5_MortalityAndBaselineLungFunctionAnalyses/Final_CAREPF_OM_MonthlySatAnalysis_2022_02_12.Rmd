---
title: "Monthly Satellite OM Analysis with CARE-PF Data - Updated Dichotomized OM"
...
```{r}
OM <- OM %>% 
  pivot_longer(cols=c(66:281), names_to="OM_date", names_prefix="OM_")
```
...
Partial model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ OM_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro, data=OM, id=ID)
...
Complete model
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ OM_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro + fvc_pct + dlco_pct, data=OM, id=ID)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
OMx <- OM %>% filter(!is.na(OM_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(dx_IPF) & !is.na(avg_s) & !is.na(metro))

#Then survival function
surv1 <- Surv(OMx$time_DeathTxCensor, OMx$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(OMx$OM_5yrPreCensor, df=4) + OMx$age_dx + OMx$sex + OMx$smokeHx + OMx$dich_Race + OMx$dx_IPF + OMx$avg_s + OMx$metro)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
OMx <- OM %>% filter(!is.na(OM_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(dx_IPF) & !is.na(avg_s) & !is.na(metro) & !is.na(fvc_pct) & !is.na(dlco_pct))

#Then survival function
surv1 <- Surv(OMx$time_DeathTxCensor, OMx$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(OMx$OM_5yrPreCensor, df=4) + OMx$age_dx + OMx$sex + OMx$smokeHx + OMx$dich_Race + OMx$dx_IPF + OMx$avg_s + OMx$metro + OMx$fvc_pct + OMx$dlco_pct)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Partial model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ OM5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + dx_IPF + metro, data=OM, id=ID)
...
Complete model
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ OM5yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + dx_IPF + metro + fvc_pct + dlco_pct, data=OM, id=ID)
...
Complete model
```{r}
FVC_model3 <- lm(fvc_pct ~ OM_5yrPreDx + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro, data=OM)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model3 <- lm(fvc_pct ~ OM5yr_dich + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro, data=OM)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
DLCO_model3 <- lm(dlco_pct ~ OM_5yrPreDx + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro, data=OM)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
DLCO_model3 <- lm(dlco_pct ~ OM5yr_dich + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro, data=OM)
summary(DLCO_model3)
confint(DLCO_model3)
```
