---
title: "Monthly Satellite NH4 Analysis with CARE-PF Data - Updated Dichotomized NH4"
...
```{r}
NH4 <- NH4 %>% 
  pivot_longer(cols=c(66:281), names_to="NH4_date", names_prefix="NH4_")
```
...
Partial model
```{r}
coxPH_model2 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NH4_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro, data=NH4, id=ID)
...
Complete model
```{r}
coxPH_model3 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NH4_5yrPreCensor + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro + fvc_pct + dlco_pct, data=NH4, id=ID)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
NH4x <- NH4 %>% filter(!is.na(NH4_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(dx_IPF) & !is.na(avg_s) & !is.na(metro))

#Then survival function
surv1 <- Surv(NH4x$time_DeathTxCensor, NH4x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(NH4x$NH4_5yrPreCensor, df=4) + NH4x$age_dx + NH4x$sex + NH4x$smokeHx + NH4x$dich_Race + NH4x$dx_IPF + NH4x$avg_s + NH4x$metro)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
NH4x <- NH4 %>% filter(!is.na(NH4_5yrPreCensor) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(dx_IPF) & !is.na(avg_s) & !is.na(metro) & !is.na(fvc_pct) & !is.na(dlco_pct))

#Then survival function
surv1 <- Surv(NH4x$time_DeathTxCensor, NH4x$deadORtx==1)
fit1 <- coxph(surv1 ~ pspline(NH4x$NH4_5yrPreCensor, df=4) + NH4x$age_dx + NH4x$sex + NH4x$smokeHx + NH4x$dich_Race + NH4x$dx_IPF + NH4x$avg_s + NH4x$metro + NH4x$fvc_pct + NH4x$dlco_pct)
predicted <- predict(fit1, type="terms", se.fit=T, terms=1)
...
Partial model
```{r}
coxPH_model8 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NH45yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + dx_IPF + metro, data=NH4, id=ID)
...
Complete model
```{r}
coxPH_model9 <- coxph(Surv(time_DeathTxCensor, deadORtx==1) ~ NH45yrCensor_dich + sex + age_dx + smokeHx + dich_Race + avg_s + dx_IPF + metro + fvc_pct + dlco_pct, data=NH4, id=ID)
...
Complete model
```{r}
FVC_model3 <- lm(fvc_pct ~ NH4_5yrPreDx + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro, data=NH4)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
FVC_model3 <- lm(fvc_pct ~ NH45yr_dich + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro, data=NH4)
summary(FVC_model3)
confint(FVC_model3)
```
...
Complete model
```{r}
DLCO_model3 <- lm(dlco_pct ~ NH4_5yrPreDx + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro, data=NH4)
summary(DLCO_model3)
confint(DLCO_model3)
```
...
Complete model
```{r}
DLCO_model3 <- lm(dlco_pct ~ NH45yr_dich + sex + age_dx + smokeHx + dich_Race + dx_IPF + avg_s + metro, data=NH4)
summary(DLCO_model3)
confint(DLCO_model3)
```
