---
title: "SS and fibrotic ILD PFF Complete Cohort (Consent Date Reference) Lung Function Decline Analysis - Updated Dichotomized SS"
...
```{r}
DLCO <- DLCO %>% dplyr::select(ID, SSID, TestDays, dlco_id, age_dx, sex, dich_Race, smokeHx, metro, dx_IPF, dx, dx_group, dlco_pct, first_fvc_pct, first_dlco_pct, SS_5yrPreCensor, SS5yrCensor_dich, consent_date, dlco_date, first_fvc_date, first_dlco_date, tx_date, death_date, DeathTxCensor_date)
str(DLCO)
```

```{r}
pft <- pft %>% dplyr::select(ID, SSID, fvc_id, TestDays, age_dx, sex, dich_Race, smokeHx, metro, dx_IPF, dx, dx_group, fvc_pct, first_fvc_pct, first_dlco_pct, SS_5yrPreCensor, SS5yrCensor_dich, consent_date, fvc_date, first_fvc_date, first_dlco_date, tx_date, death_date, DeathTxCensor_date)
str(pft)
```

```{r}
pft <- full_join(pft, DLCO, by=c("ID", "SSID", "TestDays", "age_dx", "sex", "dich_Race", "smokeHx", "metro", "dx_IPF", "dx", "dx_group", "first_fvc_pct", "first_dlco_pct", "SS_5yrPreCensor", "SS5yrCensor_dich", "consent_date", "first_fvc_date", "first_dlco_date", "tx_date", "death_date", "DeathTxCensor_date"))
```
...
### Base Model
```{r}
FVC_lmer2 <- lmer(fvc_pct ~ 1 + time_firstFVC*SS_5yrPreCensor + (1 + time_firstFVC| ID), data=pft)
summary(FVC_lmer2)
icc(FVC_lmer2)
report(FVC_lmer2)
```
...
### Partial Model
```{r}
FVC_lmer6 <- lmer(fvc_pct ~ 1 + time_firstFVC*SS_5yrPreCensor + time_firstFVC*sex + time_firstFVC*age_dx + time_firstFVC*smokeHx + (1 + time_firstFVC| ID), data=pft)
summary(FVC_lmer6)
icc(FVC_lmer6)
report(FVC_lmer6)
```
...
### Complete Model
```{r}
FVC_lmer7 <- lmer(fvc_pct ~ 1 + time_firstFVC*SS_5yrPreCensor + time_firstFVC*dich_Race + time_firstFVC*sex + time_firstFVC*age_dx + time_firstFVC*smokeHx + time_firstFVC*dx_IPF + time_firstFVC*metro + (1 + time_firstFVC| ID), data=pft)
summary(FVC_lmer7)
icc(FVC_lmer7)
report(FVC_lmer7)
```
...
### Base Model
```{r}
FVC_lmer2 <- lmer(fvc_pct ~ 1 + time_firstFVC*SS5yrCensor_dich + (1 + time_firstFVC| ID), data=pft)
summary(FVC_lmer2)
icc(FVC_lmer2)
report(FVC_lmer2)
```
...
### Partial Model
```{r}
FVC_lmer6 <- lmer(fvc_pct ~ 1 + time_firstFVC*SS5yrCensor_dich + time_firstFVC*sex + time_firstFVC*age_dx + time_firstFVC*smokeHx + (1 + time_firstFVC| ID), data=pft)
summary(FVC_lmer6)
icc(FVC_lmer6)
report(FVC_lmer6)
```
...
### Complete Model
```{r}
FVC_lmer7 <- lmer(fvc_pct ~ 1 + time_firstFVC*SS5yrCensor_dich + time_firstFVC*dich_Race + time_firstFVC*sex + time_firstFVC*age_dx + time_firstFVC*smokeHx + time_firstFVC*dx_IPF + time_firstFVC*metro + (1 + time_firstFVC| ID), data=pft)
summary(FVC_lmer7)
icc(FVC_lmer7)
report(FVC_lmer7)
```
...
### Base Model
```{r}
dlco_lmer2 <- lmer(dlco_pct ~ 1 + time_firstDLCO*SS_5yrPreCensor + (1 + time_firstDLCO| ID), data=pft)
summary(dlco_lmer2)
icc(dlco_lmer2)
report(dlco_lmer2)
```
...
### Partial Model
```{r}
dlco_lmer6 <- lmer(dlco_pct ~ 1 + time_firstDLCO*SS_5yrPreCensor + time_firstDLCO*sex + time_firstDLCO*age_dx + time_firstDLCO*smokeHx + (1 + time_firstDLCO | ID), data=pft)
summary(dlco_lmer6)
icc(dlco_lmer6)
report(dlco_lmer6)
```
...
### Complete Model
```{r}
dlco_lmer7 <- lmer(dlco_pct ~ 1 + time_firstDLCO*SS_5yrPreCensor + time_firstDLCO*dich_Race + time_firstDLCO*sex + time_firstDLCO*age_dx + time_firstDLCO*smokeHx + time_firstDLCO*dx_IPF + time_firstDLCO*metro + (1 + time_firstDLCO | ID), data=pft)
summary(dlco_lmer7)
icc(dlco_lmer7)
report(dlco_lmer7)
```
...
### Base Model
```{r}
DLCO_lmer2 <- lmer(dlco_pct ~ 1 + time_firstDLCO*SS5yrCensor_dich + (1 + time_firstDLCO| ID), data=pft)
summary(DLCO_lmer2)
icc(DLCO_lmer2)
report(DLCO_lmer2)
```
...
### Partial Model
```{r}
dlco_lmer6 <- lmer(dlco_pct ~ 1 + time_firstDLCO*SS5yrCensor_dich + time_firstDLCO*sex + time_firstDLCO*age_dx + time_firstDLCO*smokeHx + (1 + time_firstDLCO | ID), data=pft)
summary(dlco_lmer6)
icc(dlco_lmer6)
report(dlco_lmer6)
```
...
### Complete Model
```{r}
dlco_lmer7 <- lmer(dlco_pct ~ 1 + time_firstDLCO*SS5yrCensor_dich + time_firstDLCO*dich_Race + time_firstDLCO*sex + time_firstDLCO*age_dx + time_firstDLCO*smokeHx + time_firstDLCO*dx_IPF + time_firstDLCO*metro + (1 + time_firstDLCO | ID), data=pft)
summary(dlco_lmer7)
icc(dlco_lmer7)
report(dlco_lmer7)
```
