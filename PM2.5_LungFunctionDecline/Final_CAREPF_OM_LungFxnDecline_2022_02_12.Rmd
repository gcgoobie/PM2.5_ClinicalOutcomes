---
title: "OM and CARE-PF fILD Complete Cohort Lung Function Decline Analysis - Updated Dichotomized OM"
...
```{r}
OM <- OM %>% dplyr::select(ID, age_dx, sex, smokeHx, race, dich_Race, site, avg_s, dx_IPF, metro, province, status, txed, dx, dx_group, fvc_pct, dlco_pct, pft_timefromdx, OM_5yrPreCensor, OM5yrCensor_dich, dx_date, death_date, tx_date, DeathTxCensor_date, pft_date)
```
...
### Complete Model
```{r}
FVC_lmer3 <- lmer(fvc_pct ~ 1 + time_firstPFT*OM_5yrPreCensor + time_firstPFT*dich_Race + time_firstPFT*sex + OM_5yrPreCensor*sex + time_firstPFT*age_dx + time_firstPFT*smokeHx + time_firstPFT*avg_s + time_firstPFT*dx_IPF + time_firstPFT*metro + (1 + time_firstPFT| ID), data=pft)
summary(FVC_lmer3)
icc(FVC_lmer3)
report(FVC_lmer3)
```
...
### Complete Model
```{r}
FVC_lmer6 <- lmer(fvc_pct ~ 1 + time_firstPFT*OM5yrCensor_dich + time_firstPFT*dich_Race + time_firstPFT*sex + OM_5yrPreCensor*sex + time_firstPFT*age_dx + time_firstPFT*smokeHx + time_firstPFT*avg_s + time_firstPFT*dx_IPF + time_firstPFT*metro + (1 + time_firstPFT| ID), data=pft)
summary(FVC_lmer6)
icc(FVC_lmer6)
report(FVC_lmer6)
```
...
### Complete Model
```{r}
dlco_lmer3 <- lmer(dlco_pct ~ 1 + time_firstPFT*OM_5yrPreCensor + time_firstPFT*dich_Race + time_firstPFT*sex + time_firstPFT*age_dx + time_firstPFT*smokeHx + time_firstPFT*avg_s + time_firstPFT*dx_IPF + time_firstPFT*metro + (1 + time_firstPFT | ID), data=pft)
summary(dlco_lmer3)
icc(dlco_lmer3)
report(dlco_lmer3)
```
...
### Complete Model
```{r}
dlco_lmer6 <- lmer(dlco_pct ~ 1 + time_firstPFT*OM5yrCensor_dich + time_firstPFT*dich_Race + time_firstPFT*sex + time_firstPFT*age_dx + time_firstPFT*smokeHx + time_firstPFT*avg_s + time_firstPFT*dx_IPF + time_firstPFT*metro + (1 + time_firstPFT | ID), data=pft)
summary(dlco_lmer6)
icc(dlco_lmer6)
report(dlco_lmer6)
```
