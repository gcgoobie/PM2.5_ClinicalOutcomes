---
title: "NH4 Analysis with All Cohorts - Post-Publication Time-Weighted Analysis"
...
# Simmons - Simplifying NH4 Dataframe
```{r}
...
```{r}
IDs <- as.data.table(unique(NH4$ID))
...
NH4x <- read_excel(outfile2)
NH4x <- NH4x %>% dplyr::select(ID, SSID)
```

Merge NH4 and NH4
```{r}
NH4 <- left_join(NH4, NH4x, by="ID")
```
...
I used a inner_join here so that the complete "PFF" dataframe only includes patients with fILD that have all baseline demographics and NH4 data.
```{r}
str(PFF$ID)
PFF <- inner_join(PFF, NH4, by=c("SSID"))
```
...
# PFF - Simplifying NH4 Dataframe
Reorder so "ID" is the first column
```{r}
NH4 <- NH4 %>% dplyr::select(ID, everything(.))
NH4 <- NH4 %>% dplyr::select(!SSID)
```
...
# CARE-PF - Simplifying NH4 Dataframe
```{r}
NH4 <- NH4 %>% dplyr::select(!c(nrow, dist, lon, lat))
colnames(NH4)
```
...
```{r}
rm(CARE_intervals, intervals, PFF_intervals, NH4, NH4_5yrWtedAvg, NH4_5yrWtedAvgx, NH4, Simm_intervals, end, start, IDs)
```

# Make Dichotomized NH4 Cutpoints
```{r}
Simm$NH4_dich <- cut(Simm$NH4,
                      breaks=c(0, 0.31, 100),
                      labels=c("Low", "High"))
summary(Simm$NH4_dich)
class(Simm$NH4_dich)
str(Simm$NH4_dich)
```

```{r}
PFF$NH4_dich <- cut(PFF$NH4,
                      breaks=c(0, 0.31, 100),
                      labels=c("Low", "High"))
summary(PFF$NH4_dich)
class(PFF$NH4_dich)
str(PFF$NH4_dich)
```

```{r}
CARE$NH4_dich <- cut(CARE$NH4,
                      breaks=c(0, 0.31, 100),
                      labels=c("Low", "High"))
summary(CARE$NH4_dich)
class(CARE$NH4_dich)
str(CARE$NH4_dich)
```
...
Simm <- Simm %>% dplyr::select(ID, start, end, site, age_dx, sex, race, dich_Race, smokeHx, dx_IPF, disadv, dx_group, dx, dx_yr, dx_date, death_date, tx_date, DeathTxCensor_date, last_updated, fvc_date, dlco_date, fvc_pct, dlco_pct, status, deadORtx, time_DeathTxCensor, days_DeathTxCensor, event, start_5yr, end_5yr, NH4, NH4_dich)
```

```{r}
PFF <- PFF %>% rename(c("last_updated"="censor_date"))
PFF <- PFF %>% dplyr::select(ID, start, end, site, age_dx, sex, race, dich_Race, smokeHx, dx_IPF, disadv, dx_group, dx, dx_yr, dx_date, death_date, tx_date, DeathTxCensor_date, last_updated, fvc_date, dlco_date, fvc_pct, dlco_pct, status, deadORtx, time_DeathTxCensor, days_DeathTxCensor, event, start_5yr, end_5yr, NH4, NH4_dich)
```

```{r}
CARE <- CARE %>% mutate(dlco_date=pft_date)
CARE <- CARE %>% rename(c("fvc_date"="pft_date"))
CARE <- CARE %>% dplyr::select(ID, start, end, site, age_dx, sex, race, dich_Race, smokeHx, dx_IPF, disadv, dx_group, dx, dx_yr, dx_date, death_date, tx_date, DeathTxCensor_date, last_updated, fvc_date, dlco_date, fvc_pct, dlco_pct, status, deadORtx, time_DeathTxCensor, days_DeathTxCensor, event, start_5yr, end_5yr, NH4, NH4_dich)
```
...
Simmons
```{r}
coxPH_model1 <- coxph(Surv(start, end, event==1) ~ NH4 + dx_yr + site, data=Simm, id=ID)
summary(coxPH_model1)
```

PFF
```{r}
coxPH_model1 <- coxph(Surv(start, end, event==1) ~ NH4 + dx_yr + site, data=PFF, id=ID)
summary(coxPH_model1)
```

CARE-PF
```{r}
coxPH_model1 <- coxph(Surv(start, end, event==1) ~ NH4 + dx_yr + site, data=CARE, id=ID)
summary(coxPH_model1)
```
...
Simmons
```{r}
coxPH_model2 <- coxph(Surv(start, end, event==1) ~ NH4 + dx_yr + age_dx + sex + dich_Race + smokeHx + disadv + site, data=Simm, id=ID)
summary(coxPH_model2)
```

PFF
```{r}
coxPH_model2 <- coxph(Surv(start, end, event==1) ~ NH4 + dx_yr + age_dx + sex + dich_Race + smokeHx + disadv + site, data=PFF, id=ID)
summary(coxPH_model2)
```

CARE-PF
```{r}
coxPH_model2 <- coxph(Surv(start, end, event==1) ~ NH4 + dx_yr + age_dx + sex + dich_Race + smokeHx + disadv + site, data=CARE, id=ID)
summary(coxPH_model2)
```


### Variable HR for NH4 Continuous
Want a plot with x-axis of NH4 level and y-axis of HR - unsure if it makes sense to use these plots for the time-weighted exposure approach we have here.

Unadjusted model + dx_yr + site
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
Simmx <- Simm %>% filter(!is.na(NH4) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(site))
PFFx <- PFF %>% filter(!is.na(NH4) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(site))
CAREx <- CARE %>% filter(!is.na(NH4) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(site) & NH4!=0)

#Then make survival function
surv1 <- Surv(Simmx$start, Simmx$end, Simmx$event==1)
fit1 <- coxph(surv1 ~ pspline(Simmx$NH4, df=3) + Simmx$dx_yr + Simmx$site)
predicted1 <- predict(fit1, type="terms", se.fit=T, terms=1)

surv2 <- Surv(PFFx$start, PFFx$end, PFFx$event==1)
fit2 <- coxph(surv2 ~ pspline(PFFx$NH4, df=3) + PFFx$dx_yr + PFFx$site)
predicted2 <- predict(fit2, type="terms", se.fit=T, terms=1)

surv3 <- Surv(CAREx$start, CAREx$end, CAREx$event==1)
fit3 <- coxph(surv3 ~ pspline(CAREx$NH4, df=3) + CAREx$dx_yr + CAREx$site)
predicted3 <- predict(fit3, type="terms", se.fit=T, terms=1)
```

```{r, error=T}
#Then plot
plot(Simmx$NH4, exp(predicted1$fit), type="n", xlim=c(0,2.5), ylim=c(0,3))
lines(sm.spline(Simmx$NH4, exp(predicted1$fit)), col = "darkgoldenrod1", lty = 1 )
lines(sm.spline(Simmx$NH4, exp(predicted1$fit + 1.96 * predicted1$se)), col = "lightgoldenrod1", lty = 2 )
lines(sm.spline(Simmx$NH4, exp(predicted1$fit - 1.96 * predicted1$se)), col = "lightgoldenrod1", lty = 2 )

plot(PFFx$NH4, exp(predicted2$fit), type="n", xlim=c(0,2.5), ylim=c(0,3))
lines(sm.spline(PFFx$NH4, exp(predicted2$fit)), col = "royalblue1", lty = 1)
lines(sm.spline(PFFx$NH4, exp(predicted2$fit + 1.96 * predicted2$se)), col = "steelblue1", lty = 2 )
lines(sm.spline(PFFx$NH4, exp(predicted2$fit - 1.96 * predicted2$se)), col = "steelblue1", lty = 2 )

plot(CAREx$NH4, exp(predicted3$fit), type="n", xlim=c(0,2.5), ylim=c(0,3))
lines(sm.spline(CAREx$NH4, exp(predicted3$fit)), col = "firebrick1", lty = 1)
lines(sm.spline(CAREx$NH4, exp(predicted3$fit + 1.96 * predicted3$se)), col = "coral1", lty = 2 )
lines(sm.spline(CAREx$NH4, exp(predicted3$fit - 1.96 * predicted3$se)), col = "coral1", lty = 2 )
```
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
Simmx <- Simm %>% filter(!is.na(NH4) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(disadv) & !is.na(site))
PFFx <- PFF %>% filter(!is.na(NH4) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(disadv) & !is.na(site))
CAREx <- CARE %>% filter(!is.na(NH4) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(disadv) & !is.na(site) & NH4!=0)

#Then make survival function
surv1 <- Surv(Simmx$start, Simmx$end, Simmx$event==1)
fit1 <- coxph(surv1 ~ pspline(Simmx$NH4, df=3) + Simmx$dx_yr + Simmx$age_dx + Simmx$sex + Simmx$smokeHx + Simmx$dich_Race + Simmx$disadv + Simmx$site)
predicted1 <- predict(fit1, type="terms", se.fit=T, terms=1)

surv2 <- Surv(PFFx$start, PFFx$end, PFFx$event==1)
fit2 <- coxph(surv2 ~ pspline(PFFx$NH4, df=3) + PFFx$dx_yr + PFFx$age_dx + PFFx$sex + PFFx$smokeHx + PFFx$dich_Race + PFFx$disadv + PFFx$site)
predicted2 <- predict(fit2, type="terms", se.fit=T, terms=1)

surv3 <- Surv(CAREx$start, CAREx$end, CAREx$event==1)
fit3 <- coxph(surv3 ~ pspline(CAREx$NH4, df=3) + CAREx$dx_yr + CAREx$age_dx + CAREx$sex + CAREx$smokeHx + CAREx$dich_Race + CAREx$disadv + CAREx$site)
predicted3 <- predict(fit3, type="terms", se.fit=T, terms=1)
```
...
PFF <- PFF %>% dplyr::select(ID, start, end, site, age_dx, sex, race, dich_Race, smokeHx, dx_IPF, disadv, dx_group, dx, dx_yr, dx_date, death_date, tx_date, DeathTxCensor_date, last_updated, fvc_date, dlco_date, fvc_pct, dlco_pct, status, deadORtx, time_DeathTxCensor, days_DeathTxCensor, event, start_5yr, end_5yr, NH4, NH4_dich, cohort)
```
...
Allx <- All %>% filter(!is.na(NH4) & !is.na(start) & !is.na(end) & !is.na(event) & !is.na(dx_yr) & !is.na(cohort) & !is.na(site) & NH4!=0)
...
```{r}
#Then plot
plot(Allx$NH4, exp(predicted$fit), type="n", xlim=c(0,2.5), ylim=c(0,10))
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
Allx <- All %>% filter(!is.na(NH4) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(disadv) & !is.na(site) & NH4!=0)
...
```{r}
#Then plot
plot(Allx$NH4, exp(predicted$fit), type="n", xlim=c(0,2), ylim=c(0,10))
lines(sm.spline(Allx$NH4, exp(predicted$fit)), col = "red" , lty = 1 )
lines(sm.spline(Allx$NH4, exp(predicted$fit + 1.96 * predicted$se)), col = "orange" , lty = 2 )
lines(sm.spline(Allx$NH4, exp(predicted$fit - 1.96 * predicted$se)), col = "orange" , lty = 2 )
```
