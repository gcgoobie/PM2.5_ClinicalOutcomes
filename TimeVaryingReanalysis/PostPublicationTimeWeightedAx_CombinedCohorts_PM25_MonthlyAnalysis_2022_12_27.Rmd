---
title: "PM Analysis with All Cohorts - Post-Publication Time-Weighted Analysis"
...
```{r}
rm(CARE_intervals, intervals, PFF_intervals, PM, PM_5yrWtedAvg, PM_5yrWtedAvgx, PM25, Simm_intervals, end, start, IDs)
```

# Make Dichotomized PM2.5 Cutpoints
```{r}
Simm$PM_dich <- cut(Simm$PM,
                      breaks=c(0, 8, 100),
                      labels=c("Low", "High"))
summary(Simm$PM_dich)
class(Simm$PM_dich)
str(Simm$PM_dich)
```

```{r}
PFF$PM_dich <- cut(PFF$PM,
                      breaks=c(0, 8, 100),
                      labels=c("Low", "High"))
summary(PFF$PM_dich)
class(PFF$PM_dich)
...
```{r}
CARE$PM_dich <- cut(CARE$PM,
                      breaks=c(0, 8, 100),
                      labels=c("Low", "High"))
summary(CARE$PM_dich)
class(CARE$PM_dich)
str(CARE$PM_dich)
```
...
Simm <- Simm %>% dplyr::select(ID, start, end, site, age_dx, sex, race, dich_Race, smokeHx, dx_IPF, disadv, dx_group, dx, dx_yr, dx_date, death_date, tx_date, DeathTxCensor_date, last_updated, fvc_date, dlco_date, fvc_pct, dlco_pct, status, deadORtx, time_DeathTxCensor, days_DeathTxCensor, event, start_5yr, end_5yr, PM, PM_dich)
```

```{r}
PFF <- PFF %>% rename(c("last_updated"="censor_date"))
PFF <- PFF %>% dplyr::select(ID, start, end, site, age_dx, sex, race, dich_Race, smokeHx, dx_IPF, disadv, dx_group, dx, dx_yr, dx_date, death_date, tx_date, DeathTxCensor_date, last_updated, fvc_date, dlco_date, fvc_pct, dlco_pct, status, deadORtx, time_DeathTxCensor, days_DeathTxCensor, event, start_5yr, end_5yr, PM, PM_dich)
```

```{r}
CARE <- CARE %>% mutate(dlco_date=pft_date)
CARE <- CARE %>% rename(c("fvc_date"="pft_date"))
CARE <- CARE %>% dplyr::select(ID, start, end, site, age_dx, sex, race, dich_Race, smokeHx, dx_IPF, disadv, dx_group, dx, dx_yr, dx_date, death_date, tx_date, DeathTxCensor_date, last_updated, fvc_date, dlco_date, fvc_pct, dlco_pct, status, deadORtx, time_DeathTxCensor, days_DeathTxCensor, event, start_5yr, end_5yr, PM, PM_dich)
```
...
Simmons
```{r}
coxPH_model1 <- coxph(Surv(start, end, event==1) ~ PM + dx_yr + site, data=Simm, id=ID)
...
PFF
```{r}
coxPH_model1 <- coxph(Surv(start, end, event==1) ~ PM + dx_yr + site, data=PFF, id=ID)
summary(coxPH_model1)
```

CARE-PF
```{r}
coxPH_model1 <- coxph(Surv(start, end, event==1) ~ PM + dx_yr + site, data=CARE, id=ID)
summary(coxPH_model1)
```
...
Simmons
```{r}
coxPH_model2 <- coxph(Surv(start, end, event==1) ~ PM + dx_yr + age_dx + sex + dich_Race + smokeHx + disadv + site, data=Simm, id=ID)
summary(coxPH_model2)
```

PFF
```{r}
coxPH_model2 <- coxph(Surv(start, end, event==1) ~ PM + dx_yr + age_dx + sex + dich_Race + smokeHx + disadv + site, data=PFF, id=ID)
summary(coxPH_model2)
```

CARE-PF
```{r}
coxPH_model2 <- coxph(Surv(start, end, event==1) ~ PM + dx_yr + age_dx + sex + dich_Race + smokeHx + disadv + site, data=CARE, id=ID)
summary(coxPH_model2)
```


### Variable HR for PM Continuous
Want a plot with x-axis of PM level and y-axis of HR - unsure if it makes sense to use these plots for the time-weighted exposure approach we have here.

Unadjusted model + dx_yr + site
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
Simmx <- Simm %>% filter(!is.na(PM) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(site))
PFFx <- PFF %>% filter(!is.na(PM) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(site))
CAREx <- CARE %>% filter(!is.na(PM) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(site))

#Then make survival function
surv1 <- Surv(Simmx$start, Simmx$end, Simmx$event==1)
fit1 <- coxph(surv1 ~ pspline(Simmx$PM, df=3) + Simmx$dx_yr + Simmx$site)
predicted1 <- predict(fit1, type="terms", se.fit=T, terms=1)

surv2 <- Surv(PFFx$start, PFFx$end, PFFx$event==1)
fit2 <- coxph(surv2 ~ pspline(PFFx$PM, df=3) + PFFx$dx_yr + PFFx$site)
predicted2 <- predict(fit2, type="terms", se.fit=T, terms=1)

surv3 <- Surv(CAREx$start, CAREx$end, CAREx$event==1)
fit3 <- coxph(surv3 ~ pspline(CAREx$PM, df=3) + CAREx$dx_yr + CAREx$site)
predicted3 <- predict(fit3, type="terms", se.fit=T, terms=1)
```

```{r}
#Then plot
plot(Simmx$PM, exp(predicted1$fit), type="n", xlim=c(0,20), ylim=c(0.6,2.5))
lines(sm.spline(Simmx$PM, exp(predicted1$fit)), col = "darkgoldenrod1", lty = 1 )
lines(sm.spline(Simmx$PM, exp(predicted1$fit + 1.96 * predicted1$se)), col = "lightgoldenrod1", lty = 2 )
lines(sm.spline(Simmx$PM, exp(predicted1$fit - 1.96 * predicted1$se)), col = "lightgoldenrod1", lty = 2 )

plot(PFFx$PM, exp(predicted2$fit), type="n", xlim=c(0,20), ylim=c(0.6,2.5))
lines(sm.spline(PFFx$PM, exp(predicted2$fit)), col = "royalblue1", lty = 1)
lines(sm.spline(PFFx$PM, exp(predicted2$fit + 1.96 * predicted2$se)), col = "steelblue1", lty = 2 )
lines(sm.spline(PFFx$PM, exp(predicted2$fit - 1.96 * predicted2$se)), col = "steelblue1", lty = 2 )

plot(CAREx$PM, exp(predicted3$fit), type="n", xlim=c(0,20), ylim=c(0.6,2.5))
lines(sm.spline(CAREx$PM, exp(predicted3$fit)), col = "firebrick1", lty = 1)
lines(sm.spline(CAREx$PM, exp(predicted3$fit + 1.96 * predicted3$se)), col = "coral1", lty = 2 )
lines(sm.spline(CAREx$PM, exp(predicted3$fit - 1.96 * predicted3$se)), col = "coral1", lty = 2 )
```
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
Simmx <- Simm %>% filter(!is.na(PM) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(disadv) & !is.na(site))
PFFx <- PFF %>% filter(!is.na(PM) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(disadv) & !is.na(site))
CAREx <- CARE %>% filter(!is.na(PM) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(disadv) & !is.na(site))

#Then make survival function
surv1 <- Surv(Simmx$start, Simmx$end, Simmx$event==1)
fit1 <- coxph(surv1 ~ pspline(Simmx$PM, df=3) + Simmx$dx_yr + Simmx$age_dx + Simmx$sex + Simmx$smokeHx + Simmx$dich_Race + Simmx$disadv + Simmx$site)
predicted1 <- predict(fit1, type="terms", se.fit=T, terms=1)

surv2 <- Surv(PFFx$start, PFFx$end, PFFx$event==1)
fit2 <- coxph(surv2 ~ pspline(PFFx$PM, df=3) + PFFx$dx_yr + PFFx$age_dx + PFFx$sex + PFFx$smokeHx + PFFx$dich_Race + PFFx$disadv + PFFx$site)
predicted2 <- predict(fit2, type="terms", se.fit=T, terms=1)

surv3 <- Surv(CAREx$start, CAREx$end, CAREx$event==1)
fit3 <- coxph(surv3 ~ pspline(CAREx$PM, df=3) + CAREx$dx_yr + CAREx$age_dx + CAREx$sex + CAREx$smokeHx + CAREx$dich_Race + CAREx$disadv + CAREx$site)
predicted3 <- predict(fit3, type="terms", se.fit=T, terms=1)
```
...
```{r}
#Plotting in an overlapping fashion
plot(Simmx$PM, exp(predicted1$fit), type="n", xlim=c(0,20), ylim=c(0.6,2.5))
lines(sm.spline(Simmx$PM, exp(predicted1$fit)), col = "darkgoldenrod1", lty = 1 )
lines(sm.spline(Simmx$PM, exp(predicted1$fit + 1.96 * predicted1$se)), col = "lightgoldenrod1", lty = 2 )
lines(sm.spline(Simmx$PM, exp(predicted1$fit - 1.96 * predicted1$se)), col = "lightgoldenrod1", lty = 2 )
...
PFF <- PFF %>% dplyr::select(ID, start, end, site, age_dx, sex, race, dich_Race, smokeHx, dx_IPF, disadv, dx_group, dx, dx_yr, dx_date, death_date, tx_date, DeathTxCensor_date, last_updated, fvc_date, dlco_date, fvc_pct, dlco_pct, status, deadORtx, time_DeathTxCensor, days_DeathTxCensor, event, start_5yr, end_5yr, PM, PM_dich, cohort)
```
...
Base model
```{r}
#First need to make dataframe that doesn't have NAs in any of the required variables and which gets rid of extreme outliers (in this case chose to restrict to rows where 5yr PM is <20ug/m3)
Allx <- All %>% filter(!is.na(PM) & !is.na(ID) & !is.na(start) & !is.na(end) & !is.na(event) & !is.na(dx_yr) & !is.na(cohort) & !is.na(site) & PM<=20)

#Then make survival function
surv1 <- Surv(Allx$start, Allx$end, Allx$event==1)
fit1 <- coxph(surv1 ~ pspline(Allx$PM, df=3) + Allx$dx_yr + cluster(Allx$cohort) + Allx$site, id=Allx$ID)
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
Allx <- All %>% filter(!is.na(PM) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(disadv) & !is.na(site) & PM<=20)
...
# Visual EDA
Overlapping histograms of PM2.5 exposure by cohort
```{r}
(All %>% ggplot(aes(x=PM, color=cohort))+
   geom_density(alpha=0.7, aes(fill=cohort))+
   labs(x="PM2.5 Estimates", y="Density of Patient Exposures with this PM2.5 Level", title="PM2.5 Density Plot")+
