---
title: "BC Analysis with All Cohorts - Post-Publication Time-Weighted Analysis"
...
# Simmons - Simplifying BC Dataframe
```{r}
...
```{r}
IDs <- as.data.table(unique(BC$ID))
...
BCx <- read_excel(outfile2)
BCx <- BCx %>% dplyr::select(ID, SSID)
```

Merge BC and BC
```{r}
BC <- left_join(BC, BCx, by="ID")
```
...
I used a inner_join here so that the complete "PFF" dataframe only includes patients with fILD that have all baseline demographics and BC data.
```{r}
str(PFF$ID)
PFF <- inner_join(PFF, BC, by=c("SSID"))
```
...
# PFF - Simplifying BC Dataframe
Reorder so "ID" is the first column
```{r}
BC <- BC %>% dplyr::select(ID, everything(.))
BC <- BC %>% dplyr::select(!SSID)
```
...
# CARE-PF - Simplifying BC Dataframe
```{r}
BC <- BC %>% dplyr::select(!c(nrow, dist, lon, lat))
colnames(BC)
```
...
```{r}
rm(CARE_intervals, intervals, PFF_intervals, BC, BC_5yrWtedAvg, BC_5yrWtedAvgx, BC, Simm_intervals, end, start, IDs)
```

# Make Dichotomized BC Cutpoints
```{r}
Simm$BC_dich <- cut(Simm$BC,
                      breaks=c(0, 0.31, 100),
                      labels=c("Low", "High"))
summary(Simm$BC_dich)
class(Simm$BC_dich)
str(Simm$BC_dich)
```

```{r}
PFF$BC_dich <- cut(PFF$BC,
                      breaks=c(0, 0.31, 100),
                      labels=c("Low", "High"))
summary(PFF$BC_dich)
class(PFF$BC_dich)
...
```{r}
CARE$BC_dich <- cut(CARE$BC,
                      breaks=c(0, 0.31, 100),
                      labels=c("Low", "High"))
summary(CARE$BC_dich)
class(CARE$BC_dich)
str(CARE$BC_dich)
```
...
Simm <- Simm %>% dplyr::select(ID, start, end, site, age_dx, sex, race, dich_Race, smokeHx, dx_IPF, disadv, dx_group, dx, dx_yr, dx_date, death_date, tx_date, DeathTxCensor_date, last_updated, fvc_date, dlco_date, fvc_pct, dlco_pct, status, deadORtx, time_DeathTxCensor, days_DeathTxCensor, event, start_5yr, end_5yr, BC, BC_dich)
```

```{r}
PFF <- PFF %>% rename(c("last_updated"="censor_date"))
PFF <- PFF %>% dplyr::select(ID, start, end, site, age_dx, sex, race, dich_Race, smokeHx, dx_IPF, disadv, dx_group, dx, dx_yr, dx_date, death_date, tx_date, DeathTxCensor_date, last_updated, fvc_date, dlco_date, fvc_pct, dlco_pct, status, deadORtx, time_DeathTxCensor, days_DeathTxCensor, event, start_5yr, end_5yr, BC, BC_dich)
```

```{r}
CARE <- CARE %>% mutate(dlco_date=pft_date)
CARE <- CARE %>% rename(c("fvc_date"="pft_date"))
CARE <- CARE %>% dplyr::select(ID, start, end, site, age_dx, sex, race, dich_Race, smokeHx, dx_IPF, disadv, dx_group, dx, dx_yr, dx_date, death_date, tx_date, DeathTxCensor_date, last_updated, fvc_date, dlco_date, fvc_pct, dlco_pct, status, deadORtx, time_DeathTxCensor, days_DeathTxCensor, event, start_5yr, end_5yr, BC, BC_dich)
```
...
Simmons
```{r}
coxPH_model1 <- coxph(Surv(start, end, event==1) ~ BC + dx_yr + site, data=Simm, id=ID)
summary(coxPH_model1)
```

PFF
```{r}
coxPH_model1 <- coxph(Surv(start, end, event==1) ~ BC + dx_yr + site, data=PFF, id=ID)
summary(coxPH_model1)
```

CARE-PF
```{r}
coxPH_model1 <- coxph(Surv(start, end, event==1) ~ BC + dx_yr + site, data=CARE, id=ID)
summary(coxPH_model1)
```
...
Simmons
```{r}
coxPH_model2 <- coxph(Surv(start, end, event==1) ~ BC + dx_yr + age_dx + sex + dich_Race + smokeHx + disadv + site, data=Simm, id=ID)
summary(coxPH_model2)
```

PFF
```{r}
coxPH_model2 <- coxph(Surv(start, end, event==1) ~ BC + dx_yr + age_dx + sex + dich_Race + smokeHx + disadv + site, data=PFF, id=ID)
summary(coxPH_model2)
```

CARE-PF
```{r}
coxPH_model2 <- coxph(Surv(start, end, event==1) ~ BC + dx_yr + age_dx + sex + dich_Race + smokeHx + disadv + site, data=CARE, id=ID)
summary(coxPH_model2)
```


### Variable HR for BC Continuous
Want a plot with x-axis of BC level and y-axis of HR - unsure if it makes sense to use these plots for the time-weighted exposure approach we have here.

Unadjusted model + dx_yr + site
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
Simmx <- Simm %>% filter(!is.na(BC) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(site))
PFFx <- PFF %>% filter(!is.na(BC) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(site))
CAREx <- CARE %>% filter(!is.na(BC) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(site))

#Then make survival function
surv1 <- Surv(Simmx$start, Simmx$end, Simmx$event==1)
fit1 <- coxph(surv1 ~ pspline(Simmx$BC, df=3) + Simmx$dx_yr + Simmx$site)
predicted1 <- predict(fit1, type="terms", se.fit=T, terms=1)

surv2 <- Surv(PFFx$start, PFFx$end, PFFx$event==1)
fit2 <- coxph(surv2 ~ pspline(PFFx$BC, df=3) + PFFx$dx_yr + PFFx$site)
predicted2 <- predict(fit2, type="terms", se.fit=T, terms=1)

surv3 <- Surv(CAREx$start, CAREx$end, CAREx$event==1)
fit3 <- coxph(surv3 ~ pspline(CAREx$BC, df=3) + CAREx$dx_yr + CAREx$site)
predicted3 <- predict(fit3, type="terms", se.fit=T, terms=1)
```

```{r, error=T}
#Then plot
plot(Simmx$BC, exp(predicted1$fit), type="n", xlim=c(0,2.5), ylim=c(0,3))
lines(sm.spline(Simmx$BC, exp(predicted1$fit)), col = "darkgoldenrod1", lty = 1 )
lines(sm.spline(Simmx$BC, exp(predicted1$fit + 1.96 * predicted1$se)), col = "lightgoldenrod1", lty = 2 )
lines(sm.spline(Simmx$BC, exp(predicted1$fit - 1.96 * predicted1$se)), col = "lightgoldenrod1", lty = 2 )

plot(PFFx$BC, exp(predicted2$fit), type="n", xlim=c(0,2.5), ylim=c(0,3))
lines(sm.spline(PFFx$BC, exp(predicted2$fit)), col = "royalblue1", lty = 1)
lines(sm.spline(PFFx$BC, exp(predicted2$fit + 1.96 * predicted2$se)), col = "steelblue1", lty = 2 )
lines(sm.spline(PFFx$BC, exp(predicted2$fit - 1.96 * predicted2$se)), col = "steelblue1", lty = 2 )

plot(CAREx$BC, exp(predicted3$fit), type="n", xlim=c(0,2.5), ylim=c(0,3))
lines(sm.spline(CAREx$BC, exp(predicted3$fit)), col = "firebrick1", lty = 1)
lines(sm.spline(CAREx$BC, exp(predicted3$fit + 1.96 * predicted3$se)), col = "coral1", lty = 2 )
lines(sm.spline(CAREx$BC, exp(predicted3$fit - 1.96 * predicted3$se)), col = "coral1", lty = 2 )
```
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
Simmx <- Simm %>% filter(!is.na(BC) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(disadv) & !is.na(site))
PFFx <- PFF %>% filter(!is.na(BC) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(disadv) & !is.na(site))
CAREx <- CARE %>% filter(!is.na(BC) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(disadv) & !is.na(site))

#Then make survival function
surv1 <- Surv(Simmx$start, Simmx$end, Simmx$event==1)
fit1 <- coxph(surv1 ~ pspline(Simmx$BC, df=3) + Simmx$dx_yr + Simmx$age_dx + Simmx$sex + Simmx$smokeHx + Simmx$dich_Race + Simmx$disadv + Simmx$site)
predicted1 <- predict(fit1, type="terms", se.fit=T, terms=1)

surv2 <- Surv(PFFx$start, PFFx$end, PFFx$event==1)
fit2 <- coxph(surv2 ~ pspline(PFFx$BC, df=3) + PFFx$dx_yr + PFFx$age_dx + PFFx$sex + PFFx$smokeHx + PFFx$dich_Race + PFFx$disadv + PFFx$site)
predicted2 <- predict(fit2, type="terms", se.fit=T, terms=1)

surv3 <- Surv(CAREx$start, CAREx$end, CAREx$event==1)
fit3 <- coxph(surv3 ~ pspline(CAREx$BC, df=3) + CAREx$dx_yr + CAREx$age_dx + CAREx$sex + CAREx$smokeHx + CAREx$dich_Race + CAREx$disadv + CAREx$site)
predicted3 <- predict(fit3, type="terms", se.fit=T, terms=1)
```
...
PFF <- PFF %>% dplyr::select(ID, start, end, site, age_dx, sex, race, dich_Race, smokeHx, dx_IPF, disadv, dx_group, dx, dx_yr, dx_date, death_date, tx_date, DeathTxCensor_date, last_updated, fvc_date, dlco_date, fvc_pct, dlco_pct, status, deadORtx, time_DeathTxCensor, days_DeathTxCensor, event, start_5yr, end_5yr, BC, BC_dich, cohort)
```
...
Allx <- All %>% filter(!is.na(BC) & !is.na(start) & !is.na(end) & !is.na(event) & !is.na(dx_yr) & !is.na(cohort) & !is.na(site))
...
```{r}
#Then plot
plot(Allx$BC, exp(predicted$fit), type="n", xlim=c(0,2.5), ylim=c(0,8))
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
Allx <- All %>% filter(!is.na(BC) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(disadv) & !is.na(site))
