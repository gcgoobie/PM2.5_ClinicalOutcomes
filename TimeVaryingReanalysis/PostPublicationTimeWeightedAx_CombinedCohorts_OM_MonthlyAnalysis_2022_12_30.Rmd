---
title: "OM Analysis with All Cohorts - Post-Publication Time-Weighted Analysis"
...
# Simmons - Simplifying OM Dataframe
```{r}
...
```{r}
IDs <- as.data.table(unique(OM$ID))
...
OMx <- read_excel(outfile2)
OMx <- OMx %>% dplyr::select(ID, SSID)
```

Merge OM and OM
```{r}
OM <- left_join(OM, OMx, by="ID")
```
...
I used a inner_join here so that the complete "PFF" dataframe only includes patients with fILD that have all baseline demographics and OM data.
```{r}
str(PFF$ID)
PFF <- inner_join(PFF, OM, by=c("SSID"))
```
...
# PFF - Simplifying OM Dataframe
Reorder so "ID" is the first column
```{r}
OM <- OM %>% dplyr::select(ID, everything(.))
OM <- OM %>% dplyr::select(!SSID)
```
...
# CARE-PF - Simplifying OM Dataframe
```{r}
OM <- OM %>% dplyr::select(!c(nrow, dist, lon, lat))
colnames(OM)
```
...
```{r}
rm(CARE_intervals, intervals, PFF_intervals, OM, OM_5yrWtedAvg, OM_5yrWtedAvgx, OM, Simm_intervals, end, start, IDs)
```

# Make Dichotomized OM Cutpoints
```{r}
Simm$OM_dich <- cut(Simm$OM,
                      breaks=c(0, 2.97, 100),
                      labels=c("Low", "High"))
summary(Simm$OM_dich)
class(Simm$OM_dich)
str(Simm$OM_dich)
```

```{r}
PFF$OM_dich <- cut(PFF$OM,
                      breaks=c(0, 2.97, 100),
                      labels=c("Low", "High"))
summary(PFF$OM_dich)
class(PFF$OM_dich)
...
```{r}
CARE$OM_dich <- cut(CARE$OM,
                      breaks=c(0, 2.97, 100),
                      labels=c("Low", "High"))
summary(CARE$OM_dich)
class(CARE$OM_dich)
str(CARE$OM_dich)
```
...
Simm <- Simm %>% dplyr::select(ID, start, end, site, age_dx, sex, race, dich_Race, smokeHx, dx_IPF, disadv, dx_group, dx, dx_yr, dx_date, death_date, tx_date, DeathTxCensor_date, last_updated, fvc_date, dlco_date, fvc_pct, dlco_pct, status, deadORtx, time_DeathTxCensor, days_DeathTxCensor, event, start_5yr, end_5yr, OM, OM_dich)
```

```{r}
PFF <- PFF %>% rename(c("last_updated"="censor_date"))
PFF <- PFF %>% dplyr::select(ID, start, end, site, age_dx, sex, race, dich_Race, smokeHx, dx_IPF, disadv, dx_group, dx, dx_yr, dx_date, death_date, tx_date, DeathTxCensor_date, last_updated, fvc_date, dlco_date, fvc_pct, dlco_pct, status, deadORtx, time_DeathTxCensor, days_DeathTxCensor, event, start_5yr, end_5yr, OM, OM_dich)
```

```{r}
CARE <- CARE %>% mutate(dlco_date=pft_date)
CARE <- CARE %>% rename(c("fvc_date"="pft_date"))
CARE <- CARE %>% dplyr::select(ID, start, end, site, age_dx, sex, race, dich_Race, smokeHx, dx_IPF, disadv, dx_group, dx, dx_yr, dx_date, death_date, tx_date, DeathTxCensor_date, last_updated, fvc_date, dlco_date, fvc_pct, dlco_pct, status, deadORtx, time_DeathTxCensor, days_DeathTxCensor, event, start_5yr, end_5yr, OM, OM_dich)
```
...
Simmons
```{r}
coxPH_model1 <- coxph(Surv(start, end, event==1) ~ OM + dx_yr + site, data=Simm, id=ID)
summary(coxPH_model1)
```

PFF
```{r}
coxPH_model1 <- coxph(Surv(start, end, event==1) ~ OM + dx_yr + site, data=PFF, id=ID)
summary(coxPH_model1)
```

CARE-PF
```{r}
coxPH_model1 <- coxph(Surv(start, end, event==1) ~ OM + dx_yr + site, data=CARE, id=ID)
summary(coxPH_model1)
```
...
Simmons
```{r}
coxPH_model2 <- coxph(Surv(start, end, event==1) ~ OM + dx_yr + age_dx + sex + dich_Race + smokeHx + disadv + site, data=Simm, id=ID)
summary(coxPH_model2)
```

PFF
```{r}
coxPH_model2 <- coxph(Surv(start, end, event==1) ~ OM + dx_yr + age_dx + sex + dich_Race + smokeHx + disadv + site, data=PFF, id=ID)
summary(coxPH_model2)
```

CARE-PF
```{r}
coxPH_model2 <- coxph(Surv(start, end, event==1) ~ OM + dx_yr + age_dx + sex + dich_Race + smokeHx + disadv + site, data=CARE, id=ID)
summary(coxPH_model2)
```


### Variable HR for OM Continuous
Want a plot with x-axis of OM level and y-axis of HR - unsure if it makes sense to use these plots for the time-weighted exposure approach we have here.

Unadjusted model + dx_yr + site
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
Simmx <- Simm %>% filter(!is.na(OM) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(site))
PFFx <- PFF %>% filter(!is.na(OM) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(site))
CAREx <- CARE %>% filter(!is.na(OM) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(site))

#Then make survival function
surv1 <- Surv(Simmx$start, Simmx$end, Simmx$event==1)
fit1 <- coxph(surv1 ~ pspline(Simmx$OM, df=3) + Simmx$dx_yr + Simmx$site)
predicted1 <- predict(fit1, type="terms", se.fit=T, terms=1)

surv2 <- Surv(PFFx$start, PFFx$end, PFFx$event==1)
fit2 <- coxph(surv2 ~ pspline(PFFx$OM, df=3) + PFFx$dx_yr + PFFx$site)
predicted2 <- predict(fit2, type="terms", se.fit=T, terms=1)

surv3 <- Surv(CAREx$start, CAREx$end, CAREx$event==1)
fit3 <- coxph(surv3 ~ pspline(CAREx$OM, df=3) + CAREx$dx_yr + CAREx$site)
predicted3 <- predict(fit3, type="terms", se.fit=T, terms=1)
```

```{r, error=T}
#Then plot
plot(Simmx$OM, exp(predicted1$fit), type="n", xlim=c(0,10), ylim=c(0,3))
lines(sm.spline(Simmx$OM, exp(predicted1$fit)), col = "darkgoldenrod1", lty = 1 )
lines(sm.spline(Simmx$OM, exp(predicted1$fit + 1.96 * predicted1$se)), col = "lightgoldenrod1", lty = 2 )
lines(sm.spline(Simmx$OM, exp(predicted1$fit - 1.96 * predicted1$se)), col = "lightgoldenrod1", lty = 2 )

plot(PFFx$OM, exp(predicted2$fit), type="n", xlim=c(0,10), ylim=c(0,3))
lines(sm.spline(PFFx$OM, exp(predicted2$fit)), col = "royalblue1", lty = 1)
lines(sm.spline(PFFx$OM, exp(predicted2$fit + 1.96 * predicted2$se)), col = "steelblue1", lty = 2 )
lines(sm.spline(PFFx$OM, exp(predicted2$fit - 1.96 * predicted2$se)), col = "steelblue1", lty = 2 )

plot(CAREx$OM, exp(predicted3$fit), type="n", xlim=c(0,10), ylim=c(0,3))
lines(sm.spline(CAREx$OM, exp(predicted3$fit)), col = "firebrick1", lty = 1)
lines(sm.spline(CAREx$OM, exp(predicted3$fit + 1.96 * predicted3$se)), col = "coral1", lty = 2 )
lines(sm.spline(CAREx$OM, exp(predicted3$fit - 1.96 * predicted3$se)), col = "coral1", lty = 2 )
```
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
Simmx <- Simm %>% filter(!is.na(OM) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(disadv) & !is.na(site))
PFFx <- PFF %>% filter(!is.na(OM) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(disadv) & !is.na(site))
CAREx <- CARE %>% filter(!is.na(OM) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(disadv) & !is.na(site))

#Then make survival function
surv1 <- Surv(Simmx$start, Simmx$end, Simmx$event==1)
fit1 <- coxph(surv1 ~ pspline(Simmx$OM, df=3) + Simmx$dx_yr + Simmx$age_dx + Simmx$sex + Simmx$smokeHx + Simmx$dich_Race + Simmx$disadv + Simmx$site)
predicted1 <- predict(fit1, type="terms", se.fit=T, terms=1)

surv2 <- Surv(PFFx$start, PFFx$end, PFFx$event==1)
fit2 <- coxph(surv2 ~ pspline(PFFx$OM, df=3) + PFFx$dx_yr + PFFx$age_dx + PFFx$sex + PFFx$smokeHx + PFFx$dich_Race + PFFx$disadv + PFFx$site)
predicted2 <- predict(fit2, type="terms", se.fit=T, terms=1)

surv3 <- Surv(CAREx$start, CAREx$end, CAREx$event==1)
fit3 <- coxph(surv3 ~ pspline(CAREx$OM, df=3) + CAREx$dx_yr + CAREx$age_dx + CAREx$sex + CAREx$smokeHx + CAREx$dich_Race + CAREx$disadv + CAREx$site)
predicted3 <- predict(fit3, type="terms", se.fit=T, terms=1)
```
...
PFF <- PFF %>% dplyr::select(ID, start, end, site, age_dx, sex, race, dich_Race, smokeHx, dx_IPF, disadv, dx_group, dx, dx_yr, dx_date, death_date, tx_date, DeathTxCensor_date, last_updated, fvc_date, dlco_date, fvc_pct, dlco_pct, status, deadORtx, time_DeathTxCensor, days_DeathTxCensor, event, start_5yr, end_5yr, OM, OM_dich, cohort)
```
...
Allx <- All %>% filter(!is.na(OM) & !is.na(start) & !is.na(end) & !is.na(event) & !is.na(dx_yr) & !is.na(cohort) & !is.na(site) & OM!=0)
...
```{r}
#Then plot
plot(Allx$OM, exp(predicted$fit), type="n", xlim=c(0,6), ylim=c(0,2))
...
```{r}
#First need to make dataframe that only includes patients with time_DeathTxCensor
Allx <- All %>% filter(!is.na(OM) & !is.na(time_DeathTxCensor) & !is.na(dx_yr) & !is.na(time_DeathTxCensor) & !is.na(age_dx) & !is.na(sex) & !is.na(smokeHx) & !is.na(dich_Race) & !is.na(disadv) & !is.na(site) & OM!=0)
...
```{r}
#Then plot
plot(Allx$OM, exp(predicted$fit), type="n", xlim=c(0,8), ylim=c(0,2))
lines(sm.spline(Allx$OM, exp(predicted$fit)), col = "red" , lty = 1 )
lines(sm.spline(Allx$OM, exp(predicted$fit + 1.96 * predicted$se)), col = "orange" , lty = 2 )
lines(sm.spline(Allx$OM, exp(predicted$fit - 1.96 * predicted$se)), col = "orange" , lty = 2 )
```
